...
* **排行榜 HUD**（介面佈局與顯示位置請參考 [2.2.3 戰鬥畫面 HUD 介面佈局示意圖](#223-戰鬥畫面-hud-介面佈局示意圖)）：
...
對局結束後，系統彈出對局結算畫面（介面佈局請參考 [2.2.4 遊戲結算畫面佈局示意圖](#224-遊戲結算畫面佈局示意圖)），條列式展示以下局內遊戲內容（金幣、熟練度與成長之路進度等局外養成數值不在此畫面上顯示，改由大廳或結算後的專屬流程發放）： 📝[v13.0]
...
當玩家關閉「對局結算」視窗後，系統會自動載入「平台結算介面」（介面佈局與按鈕引導請參考 [2.2.5 平台結算畫面佈局示意圖](#225-平台結算畫面-佈局示意圖)）進行局外數值發放與活動結算：
...
| **吸附生效距離** | `蛇頭碰撞半徑 + 食物碰撞半徑 + 2 px` | **判定食物已被吃掉的最小距離閾值**。當飛向蛇頭的食物與蛇頭中心的距離小於或等於此數值時，立即觸發吞食判定（食物銷毀、積分增加、長度增長）。此設計是為了避免蛇隻在高速衝刺時，因影格渲染的時間差而穿透了食物卻無法觸發正常吃食的問題。 |
...
| `ENTRY_ENERGY_TEAM`  |    30    |  點  | 多人組隊賽入場參賽能量門票                       |
