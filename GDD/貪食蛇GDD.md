...
**文件版本**：21.0
**更新日期**：2026-07-07
**本次更新**：主要針對 Ch9 (AI 機制) 的修正紀錄，以及優化全域公式語法與 Markdown 顯示。
...
* **Ch9 AI 機制與轉向公式修正**：
  * 修正並簡化 AI 轉彎精準度 (`SteeringPrecision`) 計算模型，替換原本繁瑣且具座標依賴的四步法公式，採用更簡潔、座標無關的直接向量偏角模型。
  * 明確定義最大偏差角度與實際轉角之 LaTeX 數學公式，使外包開發與程式實做邏輯更加嚴緊且易於理解。
* **Ch7.4 長度成長關係公式修正**：
  * 修正長度成長換算公式，將原本的 `floor` 修正為 `int` 並註記為無條件捨去至整數，以配合程式標準語法。
* **全域公式與邏輯語法優化**：
  * 將規格書中所有數學公式與邏輯判定公式統一標準化，全面替換為專業的 LaTeX 數學區塊，並使用 `> [!IMPORTANT]` 區塊加亮顯示。
  * 對 LaTeX 公式語法進行優化（如使用 `\text{}` 處理中文與變數名稱、優化多行公式對齊等），確保在跨平台 Markdown 閱讀器中的渲染效果與一致性。
...
> [!IMPORTANT]
> $$
> \text{單局基礎熟練度} = \text{該對局模式之入場能量門票} \times 10
> $$
...
> [!IMPORTANT]
> $$
> \text{單局最終獲得熟練度} = \text{基礎熟練度} + \text{評級額外加成}
> $$
...
> [!IMPORTANT]
> $$
> \text{該區域實際比例} = \text{該區域權重} / \text{當前所有有效區域權重總和}
> $$
...
> [!IMPORTANT]
> $$
> \text{吞食得分} = \text{食物基礎價值} \times \text{ItemCandyMultiplier (幸運糖得分倍率)}
> $$
...
> [!IMPORTANT]
> $$
> \text{目前蛇隻總節數} = \text{初始長度} + \text{int}\left( \frac{\text{本次生命週期累積吞食積分 X}}{\text{每節所需積分}} \right)（無條件捨去至整數）
> $$
...
> [!IMPORTANT]
> $$
> \text{第 } N \text{ 次使用需要消耗的金幣} = \text{int}(\text{基礎價格} \times (1 + ((\text{高級道具價格倍率} / 10000) - 1) \times (N - 1)))
> $$
...
> **第一步：計算最大偏差角度**
> $$
> \text{最大偏差角度} = 180^\circ \times \left(1 - \frac{\text{轉彎精準度}}{10000}\right)
> $$
>
...
> **第二步：計算實際轉角**
> $$
> \text{實際角度} = \text{目標角度} + \text{隨機}(-\text{最大偏差角度}, \text{最大偏差角度})
> $$
>
> * **實際角度**（`ActualAngle`）：最終送入移動系統並執行的實際轉向角度。
> * **目標角度**（`TargetAngle`）：AI 尋路計算出來的理論完美角度。
> * **隨機** (`min`, `max`)：產生一個介於 min 與 max 之間的隨機實數值（角度制，若使用弧度則上限對應 $\pi$ 弧度）。
...
>
> 範例：收集型 AI 的 `SteeringPrecision = 8000 (80%)` 時，若理論避險角度為 45°，實際指令方向會隨機在 9°~81° 之間偏航（最大偏差 36°），使軌跡呈現人為手操的隨機平滑弧線。
>
