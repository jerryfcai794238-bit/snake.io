...
- 蛇隻吃食物增加長度後，依目前蛇身節數**提高自然體型倍率**；頭部、身體的**渲染尺寸與碰撞半徑同步放大**。
...
本節先說明外包實作需要的輸入、計算流程與輸出；本節只列新增參數，既有參數沿用原章節與 Module 定義。

**輸入**

- 目前蛇身節數。
- 既有初始蛇身節數、頭部／身體基礎碰撞半徑、蛇節中心間距與相機基礎視野倍率。

既有參數不在本節重複列出：初始蛇身節數與基礎碰撞半徑沿用 [Ch4.2 初始蛇隻基礎屬性規格表](#42-初始蛇隻基礎屬性規格表)，蛇節中心間距沿用 [Ch4.5 蛇隻視覺節奏與尺寸對照表](#45-蛇隻視覺節奏與尺寸對照表)，相機基礎視野倍率沿用 [Ch16.1 Module](#161-module)。

**計算流程**
- 目前蛇身節數 → 自然體型倍率 → 渲染／碰撞尺寸 → 成長視野
...
自然體型倍率 = 初始體型倍率 + (當前蛇身節數 - 初始蛇身節數) × 每節體型倍率（結果不低於初始倍率，且不超過上限）
...
> ```text
> 超出初始長度的節數
> = max(目前蛇身節數 - BaseSnakeSegment, 0)
>
> 自然體型倍率
> = min(
>     10000 + 超出初始長度的節數 * SnakeBodyGrowthScalePerSegment,
>     SnakeBodyGrowthScaleMax
>   )
> ```
>
> **預設值**
...
- 渲染尺寸與碰撞半徑共用同一個自然體型倍率，並在 `250ms` 內使用同一個線性插值值。
- 基礎半徑與節點間距見 [Ch4.5 蛇隻視覺節奏與尺寸對照表](#45-蛇隻視覺節奏與尺寸對照表)；長度變化依 [Ch7.4 長度成長關係](#74-長度成長關係)，碰撞後果依 [Ch5.1 生死與碰撞判定矩陣](#51-生死與碰撞判定矩陣)。

以下為公式套用結果：

| 蛇身節數 | 自然體型倍率 | 頭部碰撞半徑 | 身體碰撞半徑 |
| ---: | ---: | ---: | ---: |
| `50` | `1.000x` | `12px` | `8px` |
| `100` | `1.125x` | `14px` | `9px` |
