...
**文件版本**：26
**更新日期**：2026-08-25
**本次更新**：補充鍵盤操作、地圖 Tile 製作與內部單位換算規格，並同步 Module／GDK 地圖尺寸及 v26 版本紀錄。
...
* **實例計算說明**：
  ...
  * 假設基礎跑速為 `4000`。
  * 當前狀態為：衝刺加成 `DashSpeedFactor = +2500`、滿級巨大蘑菇跑速 `ItemMushroomSpeed = +1500`、緩速區修正 `SlowZoneSpeedFactor = -5000`。

> [!IMPORTANT]
>
> ```text
> 速度修正總和 = 2500 + 1500 - 5000 = -1000
> 最終跑速 = 4000 × (10000 - 1000) / 10000 = 3600（等同於 36.00 px/s）
> ```
...
* 自然成長造成的體型、渲染、碰撞與體型跟隨鏡頭只在蛇身達 `100／150／200／250／300` 節時更新；`50–99` 節維持初始體型與鏡頭。
...
| :---: | :--- | :--- | ---: | :--- |
| 80 | `SnakeBodyGrowthStageSegment` | 每個自然體型階段的節數。 | `50` | 節 |
| 81 | `SnakeBodyGrowthStageScaleIncrement` | 每個自然體型階段的倍率增量。 | `1000` | 萬分比 |
| 82 | `SnakeBodyGrowthScaleMax` | 自然體型倍率上限。 | `15000` | 萬分比 |
| 83 | `SnakeBodyGrowthScaleMaxThresholdSegment` | 自然體型倍率達到上限的門檻節數；不限制蛇身繼續成長。 | `300` | 節 |
| 84 | `SnakeBodyScaleTransitionTime` | 體型渲染尺寸、碰撞半徑與體型跟隨鏡頭的過渡時間。 | `250` | ms |
...
以上新增參數的完整型別與數值見 [Ch16.1 Module](#161-module) 的 ID 80–87。
...
>
> ```text
> 若目前蛇身節數 ≥ 自然體型倍率上限門檻節數 300（SnakeBodyGrowthScaleMaxThresholdSegment）：自然體型倍率 = 自然體型倍率上限 15000（SnakeBodyGrowthScaleMax）。
> 若目前蛇身節數 < 自然體型倍率上限門檻節數 300（SnakeBodyGrowthScaleMaxThresholdSegment）：自然體型倍率 = 初始體型倍率 10000 + int(max(目前蛇身節數 - 初始蛇身節數 50（BaseSnakeSegment）, 0) / 每階段節數 50（SnakeBodyGrowthStageSegment）) × 每階段自然體型倍率增量 1000（SnakeBodyGrowthStageScaleIncrement）。
> ```
>
> 初始體型倍率為 `10000 = 1.000x`。
> 因此自然體型只會在 `100／150／200／250／300` 節更新為 `1.100x／1.200x／1.300x／1.400x／1.500x`；
> 低於 `100` 節維持 `1.000x`，蛇身超過 `300` 節後仍維持 `1.500x`。
...
* 跨越體型階段時，渲染尺寸、碰撞半徑與體型跟隨鏡頭以 `250ms` 從目前插值值線性過渡至新目標；碰撞判定採用當前插值半徑。
* 過渡期間再次跨越體型階段時，從目前插值值重新過渡至新目標。

##### 4.7.3 體型跟隨鏡頭

鏡頭依目前體型階段的最終體型倍率計算，並在跨入體型階段或巨大蘑菇開始／結束時，以 `250ms` 平滑過渡。
各階段的實際鏡頭倍率見下表。
...
當最終體型變化時，體型跟隨鏡頭使用同一個 `250ms` 的當前插值體型逐幀計算，不另行播放獨立鏡頭動畫。

以下為預設值的完整階段驗收結果；鷹眼使用滿級 `1.750x`，巨大蘑菇使用滿級 `1.500x`：

| 節數 | 自然體型 | 自然鏡頭 | 自然＋鷹眼鏡頭 | 蘑菇體型 | 蘑菇鏡頭 | 蘑菇＋鷹眼鏡頭 |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| `50–99` | `1.000x` | `1.000x` | `1.750x` | `1.500x` | `1.200x` | `2.100x` |
| `100–149` | `1.100x` | `1.040x` | `1.820x` | `1.650x` | `1.260x` | `2.205x` |
| `150–199` | `1.200x` | `1.080x` | `1.890x` | `1.800x` | `1.320x` | `2.310x` |
| `200–249` | `1.300x` | `1.120x` | `1.960x` | `1.950x` | `1.380x` | `2.415x` |
| `250–299` | `1.400x` | `1.160x` | `2.030x` | `2.100x` | `1.440x` | `2.520x` |
| `≥300` | `1.500x` | `1.200x` | `2.100x` | `2.250x` | `1.500x` | `2.625x` |
...
自然體型倍率最高為 `1.500x`；巨大蘑菇的 `1.500x` 乘算後，最終體型倍率最高為 `2.250x`。
巨大蘑菇開始、結束或被覆蓋時，最終體型、渲染尺寸、碰撞半徑與體型跟隨鏡頭皆以 `250ms` 從當前插值值線性過渡至新目標。
...
> [!NOTE]
>
> 蛇身達 `300` 節以上時：無道具的體型跟隨鏡頭為 `1.200x`；僅鷹眼為 `2.100x`；僅巨大蘑菇的最終體型為 `2.250x`、體型跟隨鏡頭為 `1.500x`；巨大蘑菇加鷹眼的最終鏡頭為 `2.625x`。

**體型與鏡頭更新規則**

* 吃食物、扣長、截斷或重生時，僅於跨越體型階段時重新計算自然體型倍率；巨大蘑菇開始、結束或被覆蓋時，直接依目前體型階段重新計算最終體型倍率。
* 跨越體型階段或巨大蘑菇狀態變化時，渲染、碰撞與體型跟隨鏡頭在同一事件、同一幀開始 `250ms` 過渡；過渡期間再次變化時，從當前插值值重新設定目標。
