...
**文件版本**：24
**更新日期**：2026-08-12
**本次更新**：新增蛇隻體型成長、動態碰撞半徑與成長視野規格，並將地圖調整為 2000 × 2000px。
...
<details open>
<summary><b>v24.0 (2026-08-12)</b></summary>

體型成長與地圖尺寸同步

* **地圖尺寸**：`Ch6.1` 與 `Ch16.1 Module` 將地圖寬高由 `2200px` 調整為 `2000px`。
* **體型成長規格**：新增 `Ch4.7`，定義自然體型倍率、巨大蘑菇疊加、碰撞半徑、成長視野與體型／視野橫向對照表。
* **事件與過渡**：定義長度增減、巨大蘑菇啟用／到期、重生時的重新計算與過渡時間。
* **規格引用整理**：`Ch7.4` 僅保留長度規則；`Ch4.2`、`Ch4.5`、`Ch5.1`、`Ch8.1` 改以索引引用 `Ch4.7`，避免重複定義。
* **Module 參數**：新增 ID `79–84`，補充體型增量、倍率上限、碰撞過渡與成長視野參數描述。
* **Ch4.7 格式與索引整理**：統一 `[!IMPORTANT]` 公式區塊、`[!NOTE]` 範例格式與體型／視野橫向表格，並同步修正章節分隔線與全文件 Ch4.7 連結。
...
| 項目 | 建議數值 | 說明 |
| :--- | :---: | :--- |
| **蛇節視覺間距** | `10 px` | 頭部與第一節，以及每節之間的固定螢幕渲染距離；不隨體型倍率增加。 |
...
蛇隻吃食物增加長度後，依目前蛇身節數**提高體型倍率**；頭部、身體的**渲染尺寸與碰撞半徑同步放大**。
計算順序為：蛇身節數 → 自然體型倍率 → 巨大蘑菇後的有效體型倍率 → 渲染與碰撞尺寸 → 成長視野。
玩家蛇與電腦 AI 使用相同計算流程。
...
| 名稱 | 用途 | 預設值 | 參數來源 |
| :--- | :--- | :--- | :--- |
| 初始蛇身節數（`BaseSnakeSegment`） | 從這個節數開始計算體型增加。 | `50` 節 | Ch4.2 初始蛇隻基礎屬性規格表 |
| 每增加一節的體型倍率增量（`SnakeBodyGrowthScalePerSegment`） | 蛇身每增加一節，體型倍率增加的數值。 | `25 = 0.25%／節` | 新增 |
| 自然體型倍率上限（`SnakeBodyGrowthScaleMax`） | 蛇隻不使用道具時可達到的最大體型。 | `16000 = 1.600x` | 新增 |
| 體型變化過渡時間（`SnakeBodyScaleTransitionTime`） | 渲染尺寸與碰撞半徑從舊值變到新值所需的動畫時間。 | `250ms` | 新增 |
| 成長視野跟隨比例（`SnakeGrowthVisionFollowRatio`） | 體型增加時，視野跟著增加的比例。 | `4000 = 40%` | 新增 |
| 成長視野倍率上限（`SnakeGrowthVisionScaleMax`） | 成長造成的視野最大倍率。 | `12500 = 1.250x` | 新增 |
| 成長視野過渡時間（`SnakeGrowthVisionTransitionTime`） | 視野從舊倍率變到新倍率所需的動畫時間。 | `500ms` | 新增 |

以上新增參數的完整型別與數值見 [Ch16.1 Module](#161-module)。

##### 4.7.2 體型倍率與碰撞尺寸

自然體型倍率 = 初始體型倍率 + (當前蛇身節數 - 初始蛇身節數) × 每節體型倍率增量（結果不低於初始倍率，且不超過上限）
...
> ```text
> 自然體型倍率
> = min(10000 + max((目前蛇身節數 - BaseSnakeSegment), 0)
> * SnakeBodyGrowthScalePerSegment, SnakeBodyGrowthScaleMax)
> ```
>
> **預設值**
>
> - 初始蛇身節數（`BaseSnakeSegment`）：`50` 節。
> - 初始體型倍率：`10000 = 1.000x`。
> - 每增加一節的體型倍率增量（`SnakeBodyGrowthScalePerSegment`）：`25 = 0.25%／節`。
> - 自然體型倍率上限（`SnakeBodyGrowthScaleMax`）：`16000 = 1.600x`。
> - 達到上限的蛇身節數：`290` 節。
...
>
> ```text
> 有效體型倍率 = int(自然體型倍率 * 當前巨大化倍率 / 10000 + 0.5)
> ```
>
> **碰撞半徑**
>
> ```text
> 當前碰撞半徑 = int(基礎碰撞半徑 * 有效體型倍率 / 10000 + 0.5)
> ```
>
> 渲染尺寸同樣使用有效體型倍率，與碰撞半徑同步變化。

頭部基礎半徑為 `12px`、身體基礎半徑為 `8px`；蛇節中心間距固定為 `10px`。
吃食物、扣長、截斷、巨大蘑菇狀態改變或重生後，重新計算體型與碰撞尺寸。
體型變大或縮小時，以 `250ms` 播放由舊尺寸平滑變為新尺寸的線性放大／縮小動畫。

以下為公式套用結果：

| 蛇身節數 | 自然體型倍率 | 頭部半徑 | 身體半徑 |
| ---: | ---: | ---: | ---: |
| `50` | `1.000x` | `12px` | `8px` |
| `100` | `1.125x` | `14px` | `9px` |
| `150` | `1.250x` | `15px` | `10px` |
| `200` | `1.375x` | `17px` | `11px` |
| `250` | `1.500x` | `18px` | `12px` |
| `290` | `1.600x` | `19px` | `13px` |
...
>
> ```text
> 成長視野倍率 = min(10000 + max(有效體型倍率 - 10000, 0) * SnakeGrowthVisionFollowRatio / 10000, SnakeGrowthVisionScaleMax)
> 最終視野倍率 = max(10000, 成長視野倍率, 當前ItemEyeScale)
> ```
>
> **預設值**
>
...
>
> 鷹眼倍率與成長視野取較大值，不互相相乘；鷹眼原始參數沿用 [Ch8.1 高級道具](#81-高級道具)。

以下表格為無巨大蘑菇、無鷹眼時的體型與視野倍率；例如 `1.125x` 代表體型放大 `12.5%`，`1.050x` 代表鏡頭拉遠 `5%`：
...
> [!NOTE]
> 例：若 `290` 節時啟用巨大蘑菇，有效體型倍率為 `2.400x`，視野倍率仍受上限限制為 `1.250x`。
...
| ![巨大蘑菇](<./Reference Image/Ref_Icon_道具-巨大蘑菇.png>) | **巨大蘑菇 <br> (Giant Mushroom)** | 啟動巨大化效果，同時提升蛇隻渲染尺寸與碰撞半徑，並降低衝刺速度，強調身體放大與高速移動感。 | 巨大化尺寸：`ItemMushroom_Size = 15000` (1.5x，套用方式見 [Ch4.7](#47-蛇隻體型成長與碰撞半徑)) <br> 跑速變化：`ItemMushroomSpeed = -2500` (-25%) | 10s <br> (由 `ItemMushroomDuration` 控制) |
...
|  | 82 | ui | 有效體型倍率反映至成長視野的比例 | GROWTH_VISION_FOLLOW_RATIO | SnakeGrowthVisionFollowRatio | 4000 | 萬分比 |
