...
**文件版本**：27
**更新日期**：2026-09-01
**本次更新**：同步長度積分與體型成長參數；移除自然體型倍率門檻節數，改由自然體型倍率上限封頂，並更新 v27 版本紀錄。
...
* **遊戲世界尺寸**：寬度 `MapWidth = 2048`、高度 `MapHeight = 2048`（由 [Ch16.1 Module](#161-module) 控制）。兩者皆為遊戲世界的內部座標，沿用既有世界規格的 `px` 欄位語意，不代表螢幕實際像素數。
...
> 完整地圖美術單邊尺寸 = 2048 個內部單位 × 3.75 = 7680px
> 單邊 Tile 格數 = 7680px ÷ 128px = 60 格
> 完整地圖 Tile 格數 = 60 × 60 = 3600 格
> ```

| 製作項目 | 規格 | 說明 |
| :--- | :---: | :--- |
| 遊戲世界尺寸 | `2048 × 2048` | 遊戲內部座標，不是螢幕像素。 |
| 完整地圖美術尺寸 | `7680 × 7680px` | 依 `2048 × 3.75` 換算。 |
| 單格 Tile 美術尺寸 | `128 × 128px` | 由 `256 ÷ 2 = 128` 取得。 |
| 地圖 Tile 格數 | `60 × 60` | 共 `3600` 格。 |
