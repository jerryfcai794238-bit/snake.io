...
* **AI 策略與配置優化**：
  ...
  * 設定各模式對局電腦固定配比：Solo (2初階+1避險+1收集+1截斷+1追獵)；陣營對抗單側補位 (各1隻收集/截斷/追獵，無初階BOT與避險型AI)；多人大亂鬥補位 (2追獵+2截斷+2收集+1避險)。
...
* **強化消耗金幣公式 (S 型曲線)**：
  每次強化的所需金幣基於整體已強化次數（$x$，範圍 1 至 400）通過 S 型邏輯分佈曲線公式計算，計算結果四捨五入至整數：
  $$
  ...
  * **參數設定**：
    ...
    * 漲幅係數 $k = 0.024$
    * 拐點位置 $x_0 = 190$
    * 輔助係數 $A \approx -2225.11$，$B \approx 304181.64$
  * **階段特徵與關鍵節點費用**：
    * **1 ~ 100 級（甜蜜期）**：費用增長極緩慢，從 1,000 金幣升至 29,227 金幣，便於前期發育。
    * **100 ~ 200 級（陡峭段）**：費用快速增長，200 級達到 168,030，為中期的經濟回收主體。
    * **200 ~ 300 級（減速段）**：漲幅逐步放緩，300 級達到 281,696。
    * **300 ~ 400 級（收斂段）**：近乎水平，最終收斂於 300,000。
  * **扣費規則**：每次點擊隨機強化，系統隨機判定提升的屬性後，依據**當前全部屬性已強化總次數 $x$（自 1 開始，最大 400）** 代入公式扣除對應金幣。詳細對照表請參閱 [被動強化費用對照表_S型曲線.md](file:///c:/Users/fanchunkao/Documents/MyProject/snake.io/GDD/被動強化費用對照表_S型曲線.md)。
...
```text
+--------------------------------------------------------------+
| [返回]                 ★ 被動屬性強化 ★  [i]      金幣: 15,200G |
|--------------------------------------------------------------|
| [Icon] 1. 磁吸半徑 Lv.15 [███░░] | [Icon] 6. 巨大持續 Lv.12 [██░░░] |
| [Icon] 2. 巨大速跑 Lv.08 [█░░░░] | [Icon] 7. 幸運持續 Lv.02 [░░░░░] |
| [Icon] 3. 幸運食物 Lv.20 [███░░] | [Icon] 8. 鷹眼持續 Lv.15 [███░░] |
| [Icon] 4. 鷹眼視野 Lv.10 [██░░░] | [Icon] 9. 岩石抗性 Lv.30 [████░] |
| [Icon] 5. 磁鐵持續 Lv.05 [█░░░░] | [Icon] 10.河流抗性 Lv.01 [░░░░░] |
|--------------------------------------------------------------|
| 已強化次數: 114/400 次                                       |
|                                                              |
|                        [ 隨 機 強 化 ]                       |
