...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **衝刺 (Dash)** | `<Module表_基礎屬性>` | **無須解鎖** (初始 Lv 0) | 可升至 **Lv 4** (體力上限 1000~1500，回復 80~200/s)。 | 每秒消耗體力 `333` 點。走位加速比例 `+2500` (即 +25% 移動速度)。 | 體力歸 0 時進入超載狀態：(1) 蛇頭光暈變為紅色；(2) 強制鎖定禁止衝刺且暫停體力回復；(3) 體力自然回復滿至當前等級上限後解鎖。 |
| **磁力漩渦 (Vortex)** | `<Module表_基礎屬性>` | 成長之路 **Lv 1** 解鎖 | 可升至 **Lv 4** (半徑 200~400px，冷卻 30~25s)。 | 瞬間釋放磁場，以速度 `12 px/s` 吸附區域內所有食物。 | 點擊發動後進入對應冷卻遮罩。局內無次數限制。 |
...
* **單局售價遞增機制**：在單局對局中，同一個道具每被購買一次，其局內售價將上漲 **50%**（即 `1.5` 倍，由 `BOOSTER_PRICE_MULTIPLIER = 15000` 萬分比控制），用於回收金幣並限制惡意連發。
  * **遞增價格計算公式**：
    第 $N$ 次購買的價格 = 基礎價格 * (BOOSTER_PRICE_MULTIPLIER / 10000) ^ (N - 1) （計算結果無條件捨去至整數，其中 $N \ge 1$）
  * **巨大蘑菇價格遞增範例**：
    * 第 1 次購買：6000 金幣
    * 第 2 次購買：9000 金幣
    * 第 3 次購買：13500 金幣
...
* **單局使用次數限制**：
  * 單局內各道具的購買使用次數設有上限。
  * 次數上限與「成長之路等級」對接：初始（Lv 0）上限為 **2 次**，隨著成長之路等級提升，各道具的使用次數將獨立解鎖增加，最高可提升至 **5 次**（詳細解鎖等級與分配見 Ch 11.1 對照表）。
...
### 10.1 被動養成與升級公式 📝[v11.0]

* **升級消耗金幣公式**： 📝[v10.0]
  升級所需金幣 = PASSIVE_UPGRADE_BASE_COST * (PASSIVE_UPGRADE_EXPONENT / 100) ^ 當前等級 （計算結果無條件捨去至整數） 📝[v10.0]
  * **屬性等級範圍**：初始等級為 **`0`** 級（未強化狀態），最大可升至 **`40`** 級（由<Module表_被動養成>控制：`PASSIVE_MAX_LEVEL = 40`）。 📝[v11.0]

### 10.2 屬性養成明細 📝[v11.0]

| 被動屬性名稱 | 類別 | 初始(Lv 0)數值 | 滿級(Lv 40)數值 | 每級成長增量 | 作用說明 |
| :--- | :---: | :---: | :---: | :---: | :--- |
...
* 玩家可從遊戲大廳點擊「外觀商店」進入直購介面，購買已解鎖的醜萌 Skin 或表情氣泡：
  * **金幣直購 Skin**：部分常規醜萌 Skin 可消耗 `1000 ~ 5000` 金幣直接解鎖。
  * **鑽石直購 Skin**：稀有/搞怪 Skin 需消耗 `50 ~ 200` 鑽石購買。
  * **表情氣泡直購**：未解鎖表情可消耗 `20 ~ 50` 鑽石直接解鎖。
...
| 參數名稱 | 初始(Lv 0)數值 | 滿級(Lv 40)數值 | 每級成長增量 | 單位 | 參數描述 |
| :--- | :---: | :---: | :---: | :---: | :--- |
| `PASSIVE_MAGNET_UPGRADE` | 1000 | 2000 | +25 | 0.1 px | 磁吸半徑永久增量 |
| `PASSIVE_SPEED_UPGRADE` | -2500 | 1500 | +100 | 萬分比 | 巨大蘑菇跑速增幅 |
| `PASSIVE_LUCKY7_UPGRADE` | 30000 | 70000 | +1000 | 萬分比 | 幸運糖倍率永久增量 |
| `PASSIVE_EYE_UPGRADE` | 13500 | 17500 | +100 | 萬分比 | 鷹眼視野永久增量 |
| `PASSIVE_MAGNET_DUR` | 10000 | 20000 | +250 | ms | 磁鐵時長永久增量 |
| `PASSIVE_MUSHROOM_DUR` | 10000 | 20000 | +250 | ms | 巨大蘑菇時長永久增量 |
| `PASSIVE_CANDY_DUR` | 10000 | 20000 | +250 | ms | 幸運糖時長永久增量 |
| `PASSIVE_EYE_DUR` | 10000 | 20000 | +250 | ms | 鷹眼時長永久增量 |
| `PASSIVE_ROCK_RESIST` | -5000 | -1000 | +100 | 萬分比 | 岩石長度扣除減免 |
| `PASSIVE_RIVER_RESIST` | -5000 | -1000 | +100 | 萬分比 | 河流減速效果減免 |
| `PASSIVE_UPGRADE_BASE_COST` | 100 | - | - | 金幣 | 升級基礎金幣費用 |
| `PASSIVE_UPGRADE_EXPONENT` | 115 | - | - | 比例 | 升級費用指數係數 |
...
* **遊戲大廳**：左上玩家頭像，右上金幣、鑽石、能量（30 點標示）。中央橫向模式卡片（Solo、大亂鬥、組隊）。底部配置升級、商店、更換外觀及開始按鈕。
* **被動屬性升級**：列表列出 10 種屬性進度與消耗金幣升級按鈕。
