...
* **主動技能規則修正**：
  * 修正 Ch 4.2 衝刺技能，移除超載狀態中「拾取蘇打解鎖」描述。
  * 修正 Ch 4.2 暴食，移除隨成長之路解鎖次數設定，改為「解鎖並常駐升級（增加半徑、減少冷卻）」。
* **生死與碰撞規則修正**：
  * 修正 Ch 5.1 碰撞判定矩陣，雙方衝刺頭對頭碰撞改為 **「雙方皆判定死亡」**。
  * 於 Ch 5.1 碰撞判定中明確寫入碎裂殘骸僅轉化生前積分之 `DEBRIS_PERCENTAGE = 5000` (50% 萬分比) 的食物積分。
* **結構大改拆分**：
  * 將原 Ch 4 拆分為三個大章：Ch 6 地圖與環境系統、Ch 7 食物、道具與積分系統、Ch 8 局內道具與覆蓋規則。
  * 將原 7.4 中的長度與積分成長關係，單獨拉出為獨立小節 `### 7.4 長度成長關係`。
  * 後續章節編號順延至 Ch 15，並同步更新文內所有交叉參照。
...
* **公式渲染修復**：
  * 將 Ch 4.1 跑速公式由 LaTeX 方程式塊改為 Plain Text 格式，修復部分 Markdown 解析器渲染異常。
* **中離懲罰放寬**：
  * 移除 Ch 14.2 中離 5 分鐘禁止匹配限制，玩家中離後仍可立即再次發起配對。
* **版號與標記更新**：
  * 全文升級為 v0.8，清除舊 `v0.7` 標記，其餘章節全新加上 `v0.8` 標記。開頭與 Ch 0 不標註 any 標記。
...
* **參數與顯示修正**：
  * 修正 Ch 3.2 磁吸半徑顯示為純數字，移除冗餘單位。
  * 修正 Ch 3.2 岩石抗性與河流抗性數值，將「扣減/減速」描述統一改為負號（`-4900` 與 `-1000`）。
  * 修正 Ch 9.2 表情持續時長 `EMOTE_DURATION` 從 3s 調整為 `2500ms`，同步更新 Ch 15 參數總覽。
...
* **結構大改與精簡**：
  ...
  * 將碰撞生死判定、死亡復活及幽靈重生獨立為 Chapter 5，緊跟在基礎操作之後。
...
* **規則與數值微調**：
  ...
  * 將原結算評價與獎勵移入 Ch 3，刪除原經濟章節並將能量門票消耗移入 Ch 3 模式表格.
...
* **HUD 介面**：無小地圖。右上局內排行榜（4行，No.1皇冠）。正上方滑出擊殺橫幅。右下 Dash（衝刺）與暴食（半透明冷卻遮罩與倒計時）。左下虛擬搖桿。中下局內金幣消耗高級道具欄（常駐顯示鷹眼 2000G、磁鐵 4000G、巨大蘑菇 6000G、幸運糖 8000G 等道具使用按鈕）。
* **相對方位標示**：畫面四周邊緣的 crown ICON，動態指向第一名座標。
* **離開與結算**：中途離開彈窗確認。單局截止後進入「遊戲結算彈窗」，僅展示各玩家(電腦)排名、自己發育評價、擊殺數與死亡數，點擊後進入「平台結算介面」，展示平台等級、熟練度、隨機掉落獎勵、活動入口（如貼紙派對），並提供「成長之路」與「再來一局」之引導。

#### 2.2.1 遊戲大廳 UI 佈局示意圖

```text
+---------------------------------------------------------------------------------+
| [頭像] 玩家姓名  | 能量: 716⚡  星星: 50★  金幣: 142萬G                  [三] |
|---------------------------------------------------------------------------------|
|  << 貪食蛇資訊 (固定版面) >>       |  << 模式選擇與系統功能 (右側區塊) >>       |
|                                    |                                            |
|  遊戲熟練度 Lv.0                   |  +------------+  +------------+            |
|  [██████████░░░░░░] 0/30           |  |  單人對抗  |  |  [被動強化] |            |
|                                    |  |   ⚡ 30    |  +------------+            |
|  遊戲技能:                         |  +------------+  |  [外觀商店] |            |
|  [Lv.1][Lv.1][Lv.1][Lv.1][Lv.1]    |  |  多人大亂  |  +------------+            |
|                                    |  |   ⚡ 30    |  |  [更換外觀] |            |
|  獎勵一覽:                         |  +------------+  +------------+            |
|  [⭐][🐰][🐸][🐱][🐶]              |  |  陣營對抗  |  | [創建好友房] |            |
|                                    |  |   ⚡ 30    |  | [加入好友房] |            |
...
#### 2.2.3 戰鬥畫面 HUD 佈局示意圖

```text
+--------------------------------------------------------------+
| [離開]                                排行榜 (Top 4)         |
|                                       No.1 👑 AI_Master - 2500|
|       [   擊 殺 橫 幅 : 你 擊 殺 了 Bot_03 !   ]   No.2    Player_Me - 1200|
|                                       No.3    Enemy_B   - 800|
|                                       No.4    Enemy_C   - 750|
|                                                              |
|                                                              |
...
|                              ★ 對局結算 ★                              |
|-------------------------------------------------------------------------|
|  名次  |  頭像  |  名字        |  分數  |  擊殺  |  死亡  |  評價(僅自己) |
|--------+--------+--------------+--------+--------+--------+-------------|
|  No.1  |  [🐰]  |  AI_Master   |  2500  |   8    |   0    |             |
|  No.2  |  [🐸]  |  Player_Me★  |  1850  |   5    |   1    |     SS      |
|  No.3  |  [🐱]  |  Enemy_B     |   800  |   2    |   2    |             |
|  No.4  |  [🐶]  |  Enemy_C     |   750  |   1    |   3    |             |
|-------------------------------------------------------------------------|
|                               [ 返回大廳 ]                              |
+-------------------------------------------------------------------------+
```

#### 2.2.5 平台結算畫面佈局示意圖 📝[v13.0]

```text
+-------------------------------------------------------------------------------+
| [返回] 遊戲名稱 (例如: 貪食蛇)       能量: 716⚡  星星: 50★  金幣: 142萬G  [三] |
|-------------------------------------------------------------------------------|
|   +------------------------------------+   +------------------------------+   |
|   |                                    |   |          [ ★ ]               |   |
|   |             ★ 第 1 名 ★            |   |          1天7時              |   |
...
| **陣營對抗** | `ENTRY_ENERGY_TEAM = 30` 點能量 📝[v12.0] | 4 玩家 + 4 玩家 | 單局限時結束時，累計陣營總長度積分，總分高之陣營獲勝。 |
...
| **友方穿透動畫** | 隊友穿透時不播放碰撞火花或硬性擠壓反彈，僅在重合的瞬間播放極短（`100 ms`）的淡色半透明過渡光效，避免遮擋視線。 |
...
對局結束後，系統彈出對局結算畫面，條列式展示以下局內遊戲內容（金幣、熟練度與成長之路進度等局外養成數值不在此畫面上顯示，改由大廳或結算後的專屬流程發放）： 📝[v13.0]
...
根據玩家的單局結算評級發放固定金幣獎勵，與排名無直接關聯。 📝[v12.0]

#### 3.4.2 熟練度與評級加成計算 📝[v12.0]
...
* **計算公式**：
  ...
  *(例：評級為 SSS 的玩家可獲得 300 + 50 = 350 點熟練度；評級為 A 的玩家可獲得 300 點熟練度。)*

#### 3.4.3 發育評級標準 📝[v12.0]

結算發育評級（SSS 至 E 級別）完全依據玩家在單局結束時所達到的「對局總積分（積分條件）」進行評判。 📝[v12.0]

#### 3.4.4 單局結算獎勵與評級對照表 📝[v12.0]
...
#### 3.4.5 擊殺積分與結算獎勵 📝[v12.0]

對局結算時，玩家的擊殺表現將獲得額外的積分加乘與獎勵：

* **發育評級積分累加**：對局結束時，結算發育評級採用的「個人累計積分」已包含局內的所有擊殺積分（每擊殺 1 次獲得 `KILL_SCORE = 100` 分）。
* **擊殺金幣與熟練度獎勵**：每成功擊殺一隻蛇，結算時額外發放 `KILL_GOLD_BONUS = 10` 金幣與 `KILL_MASTERY_BONUS = 2` 點熟練度。此額外獎勵與評級獎勵累加發放，不設上限。 📝[v12.0]
  * *(例：玩家評級為 SSS 且擊殺 5 次，最終獲得金幣為：3000 (評級固定獎勵) + 5 * 10 = 3050 金幣；獲得熟練度為：300 (基礎) + 50 (評級加成) + 5 * 2 = 360 點。)*

#### 3.4.6 平台結算與養成機制 📝[v13.0]

當玩家關閉「對局結算」視窗後，系統會自動載入「平台結算介面」進行局外數值發放與活動結算：
...
* **隨機掉落獎勵**：對局結束時有一定機率觸發隨機物品/道具掉落。若未觸發任何掉落，底部子面板文字固定顯示 `未獲得隨機掉落獎勵`。
...
* **場景**：小型封壓沙盒（約 1200×1200 px），無敵人、無岩石、無河流。
* **目標**：引導玩家將蛇頭移動至 10 顆閃爍食物並吞食。
* **引導機制**：
  1. 畫面左下出現半透明手勢光暈，引導玩家放置左手拇指於虛擬搖桿區域。
  2. 食物高亮並出現「前往」指引虛線。
  3. 吃到第 3 顆食物後，播放 Floating Tip 提示：「移動吃食物，可增長蛇身並獲得積分！」
* **完成條件**：吃到 10 顆食物，個人長度積分達 50 分。
* **失敗條件**：無。撞牆僅會彈回，不致死亦不扣分。

##### 教學關卡 2：衝刺與河流

* **場景**：地圖中央有一條橫向河流（寬度 80 px），兩側各有 1 隻靜止的初階 BOT。
* **目標**：引導玩家使用衝刺穿過河流，並在安全區域發育。
* **引導機制**：
  1. 畫面右下角「衝刺按鈕」高亮並播放脈衝特效，提示玩家按住。
  2. 按住按鈕衝刺時，顯示體力條與加速特效。
  3. 衝刺進入河流後，播放減速漣漪特效與 🐌 狀態圖示，顯示 Floating Tip 提示：「河流會降低 50% 速度，按住衝刺可快速通過！」
* **完成條件**：成功衝刺穿越河流一次，且個人長度積分達 100 分。
* **失敗條件**：無。

##### 教學關卡 3：高級道具 - 磁鐵

* **場景**：沙盒地圖，無隨機道具，地圖中散落多個靜止食物。
* **目標**：引導玩家主動點擊 HUD 中的**磁鐵道具按鈕**（消耗金幣），體驗吸附周邊食物的效果。
* **引導機制**：
  1. 地圖上**不生成**任何實體道具。
  2. 畫面下方 HUD 的「磁鐵道具按鈕」高亮閃爍，浮空手勢指引點擊，提示 Tip：「點擊使用磁鐵，可自動吸附附近食物！」
  3. 點擊後，道具按鈕顯示剩餘次數從 `1` 變為 `0`（本局金幣充足或模擬扣減），並啟動 8 秒倒數（蛇頭上方顯示）。播放磁鐵吸力特效與音效，周圍食物向蛇頭滑行。
* **完成條件**：使用磁鐵吃到至少 20 顆被吸附食物，個人長度積分達 200 分。
* **失敗條件**：無。教學局若金幣不足，系統會自動補足，確保玩家可重複點擊。

##### 教學關卡 4：高級道具 - 鷹眼

* **場景**：沙盒地圖，隨機分散 20 顆食物。
* **目標**：引導玩家主動點擊 HUD 中的**鷹眼道具按鈕**，體驗雷達掃描與視野拉遠。
* **引導機制**：
  1. 畫面下方 HUD 的「鷹眼道具按鈕」高亮閃爍，指引點擊。
  2. 點擊後扣除模擬金幣，剩餘次數變更為 `0`。
  3. 相機相應拉遠（視野擴大至 1.35x），地圖上所有食物的位置高亮標記，持續 10 秒。
* **完成條件**：使用鷹眼掃描後，在 20 秒內吃到至少 15 顆食物，個人長度積分達 250 分。
* **失敗條件**：無。

##### 教學關卡 5：高級道具 - 巨大蘑菇

* **場景**：沙盒地圖，加入 2 隻「進階 BOT」。
* **目標**：引導玩家主動點擊 HUD 中的**巨大蘑菇道具按鈕**，化身大蛇撞擊敵方。
* **引導機制**：
  1. 畫面下方 HUD 的「巨大蘑菇道具按鈕」高亮閃爍，指引點擊。
  2. 點擊後，蛇隻體型變大（1.5 倍），外周加上金色光環無敵特效，持續 10 秒。
  3. 引導玩家在無敵狀態下衝刺去「衝撞」敵方 BOT（敵方會短暫眩暈且無法移動）。
* **完成條件**：在無敵狀態下成功衝撞敵方 BOT 至少 1 次。
* **失敗條件**：無。

##### 教學關卡 6：高級道具 - 幸運糖

* **場景**：沙盒地圖，散落少量基礎食物。
* **目標**：引導玩家主動點擊 HUD 中的**幸運糖道具按鈕**，體驗幸運時間內的食物增長與點數加成。
* **引導機制**：
  1. 畫面下方 HUD 的「幸運糖道具按鈕」高亮閃爍，指引點擊。
  2. 點擊使用後，蛇隻進入持續 15 秒的幸運狀態，食物點數獲取倍率翻倍。
* **完成條件**：在幸運糖持續時間內吃到至少 25 顆食物，個人長度積分達 300 分。
* **失敗條件**：無。

#### 3.5.4 退出與跳過機制

* **跳過教學**：關卡對局中允許玩家隨時點擊左上角「跳過」按鈕退出，彈出確認提示：「跳過教學將直接返回大廳，未通關前無法領取首次獎勵，確定跳過嗎？」。確認後退回大廳。
* **重新體驗**：通關後，玩家仍可在關卡選擇介面中自由選擇任一關卡重複體驗，但不再重複發放 FTUE 首次通關獎勵。
...
| **初始對局積分** | `500` | 分 | 初始 50 節對應的積分（每節 10 分） | `INITIAL_SCORE` |
...
* **2. 減速狀態**（跑速低於 `BASE_SPEED`）： 📝[v12.0]
  * **漣漪與阻力**：速度線與殘影消失，蛇頭四周繪製淡藍色/青色水體漣漪。 📝[v12.0]
...
| **食物碰撞半徑** | `4 px / 7 px / 10 px` | 分別對應小/中/大三種食物的物理碰撞半徑。 |
| **吸附生效距離** | `碰撞半徑 + 2 px` | 食物進入吸附半徑後的實際捕獲生效判定距離，避免高速時食物滑過蛇頭。 |

### 4.5 衝刺與減速視覺特效規範 📝[v13.0]
...
* ＜＝ 10 節 (頭部區) ➔ 受擊方死亡。**攻擊方擊殺數 +1**。
* ＞ 10 節 (身體區) ➔ 受擊方被截斷。
* **擊殺廣播提示**：當觸發 any 擊殺事件時，畫面中央上方會彈出置頂廣播提示（Tip），格式為 `"{擊殺者名稱} 擊殺 {被擊殺者名稱}"`。
...
### 5.4 死亡碎裂與殘骸生成規格 📝[v13.0]

* **殘骸尺寸**：碎裂後每段殘骸半徑固定為 `8 px`，多個殘骸生成於同一區域時，需套用隨機位置偏移 `±2 px` 以避免完全重疊。
* **殘骸價值**：轉化比例由 `DEBRIS_PERCENTAGE = 50%`（由 [16.4 地圖與場景參數](#164-地圖與場景參數) 控制）決定。當蛇隻死亡時，其個人累計長度積分的 50% 將轉化為地圖食物殘骸：
  * *(例：蛇隻死亡時長度積分為 2400 分，轉化後的殘骸總積分為 1200 分。系統將此 1200 分隨機混合生成約 120~240 顆小/中型食物。)*
* **殘骸生命週期與回收**：食物殘骸生成後不會主動消失。但當局內總食物積分達到上限時，為優化記憶體，系統會優先採用淡出（Fade-out）動畫回收生成時間最久的舊殘骸。
...
* **地形投放分配權重**：（由 [16.4 地圖與場景參數](#164-地圖與場景參數) 數據驅動）
  * **岩石區 (`FOOD_DIST_ROCK`)**：權重 `5000` (50%)。屬於高風險高回報區域，地圖道具投放密度最高，且單區域的食物總積分佔比最多。
  * **河流區 (`FOOD_DIST_RIVER`)**：權重 `3000` (30%)。具備減速阻礙阻禦，投放密度與食物總積分佔比中等。
  * **平地區 (`FOOD_DIST_PLAINS`)**：權重 `2000` (20%)。無任何環境風險，投放密度與食物總積分佔比最低。
| **首次生成 (Initial Spawn)**：每局開局時，系統在符合上述安全投放邏輯的隨機座標上，一次性生成總價值 `INITIAL_FOOD_POINTS = 2000` 分的隨機食物。
* **常態補充 (Replenish Check)**：系統每隔 `FOOD_REPLENISH_INTERVAL = 10000ms`（10秒）自動檢測一次全場數據。當地圖食物總積分低於下限值 `MIN_FOOD_POINTS = 1000` 分時，在隨機安全點補足缺額。
...
* **食物價值與尺寸渲染對照表**：
  ...
  | 食物級別                               | 基礎分數價值 | 渲染尺寸 (半徑) |
  | :------------------------------------- | :----------: | :-------------: |
  | **小食物 (`FOOD_VAL_SMALL`)**  |   `2` 分   |     `4px`     |
  | **中食物 (`FOOD_VAL_MEDIUM`)** |   `5` 分   |     `7px`     |
  | **大食物 (`FOOD_VAL_LARGE`)**  |  `10` 分  |    `10px`    |
...
* **初始長度與重置基準**：初始長度 `INITIAL_LENGTH = 50` 節（由 [16.2 基礎核心參數](#162-基礎核心參數) 控制）。當玩家死亡並重生時，蛇頭與蛇身節數立刻重置回此初始狀態。
* **長度與積分換算公式**：
  $$
  \text{最終蛇隻總節數} = \text{初始長度} + \left\lfloor \frac{\text{當前個人長度積分} - \text{重生基準分}}{\text{每節所需積分}} \right\rfloor
* **重生基準分與平衡換算**：
  * 系統預設重生基準分為 `INITIAL_SCORE = 500` 分（由 [16.2 基礎核心參數](#162-基礎核心參數) 控制）。
  * 計算公式為：$\text{初始長度 } INITIAL\_LENGTH \times \text{ 每節所需積分 } POINTS\_PER\_SECTION$
  * 依據此公式平衡，玩家在重生基準之上，每吞食累積 10 分個人長度積分即可增長 1 節長度（等同於需吞食：小食物 5 顆 / 中食物 2 顆 / 大食物 1 顆）。

### 7.5 地圖隨機道具說明

* 地圖隨機安全座標每 `ITEM_SPAWN_INTERVAL = 20000ms` 刷新一個地圖道具（由 [16.3 遊戲模式參數](#163-遊戲模式參數) 控制）。
* 地圖上道具上限為 `MAX_MAP_ITEMS = 12` 個（由 [16.3 遊戲模式參數](#163-遊戲模式參數) 控制）。
* 生成前有 `ITEM_PREVIEW_DURATION = 1500ms` 的倒數動畫預告（由 [16.3 遊戲模式參數](#163-遊戲模式參數) 控制）。

| 道具名稱                                   | 生效效果說明                                                               | 數值與半徑參數 (DataTable 綁定變數)                                                                                            |                  基礎持續時間                  |
| :----------------------------------------- | :------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------: |
...
* **次數上限解鎖**：
  ...
  * 隨大廳【成長之路】等級提升逐步解鎖，單局最高可提升至 **5 次**（詳細解鎖對照表見 Ch 11.2）。
...
* **遞增溢價邏輯**：每次使用後，下次使用所需的金幣消耗量固定增加「基礎價格 of 50%」（溢價比例由變數 `BOOSTER_PRICE_MULTIPLIER = 15000` 萬分比控制）。
* **遞增消耗計算公式**：

  $$
  \text{第 } N \text{ 次使用需要消耗的金幣} = \lfloor \text{基礎價格} + \text{基礎價格} \times (\frac{\text{BOOSTER\_PRICE\_MULTIPLIER}}{10000} - 1) \times (N - 1) \rfloor \quad (\text{其中 } N \ge 1)
  $$
* **各道具單局使用次數與金幣消耗對照表**：
...
| **B. 效果生效中** `<br>`*(道具 10 秒時效內)*      | 按鈕降低透明度（**半透明置灰 50%**）。`<br>`持續顯示剩餘次數與消耗提示。 | **【攔截點擊】**：效果期間內阻擋重複點擊與疊加消耗。不觸發效果，**不扣除金幣**。                                                                          |             `「效果生效中！」`             |
...
| 當前生效中 Buff A | 觸發事件 B | 高級按鈕狀態 | 最終效果強度 | 最終剩餘時間 | 扣除金幣/次數 | 說明與邏輯 |
| :--- | :--- | :---: | :---: | :---: | :---: | :--- |
| **無** | 撿到地圖道具 | A. 可使用 | 地圖道具強度 | 重置為 10s | 否 | 正常觸發地圖道具效果。 |
| **無** | 使用高級道具 | A. 可使用 | **Lv.40 滿級** | 重置為 10s | **是** | 正常觸發高級道具效果。 |
| **地圖道具 (非滿級)** | 撿到地圖道具 | A. 可使用 | 地圖道具強度 | **重置為 10s** | 否 | 重複拾取，刷新時長。 |
| **地圖道具 (非滿級)** | 使用高級道具 | A. 可使用 | **升級為滿級** | **重置為 10s** | **是** | **高級覆蓋普通**：套用滿級數值並重置時間。 |
| **高級道具 (滿級)** | 撿到地圖道具 | B. 效果生效中 | **維持滿級** | **重置為 10s** | 否 | **時間重置，強度不降級**（撿地圖道具續杯）。 |
| **高級道具 (滿級)** | 使用高級道具 | B. 效果生效中 | - | - | - | **點擊攔截**：不消耗金幣，不重複扣費。 |
| **滿級地圖道具 (Lv.40)** | 撿到地圖道具 | B. 效果生效中 | 維持滿級 | **重置為 10s** | 否 | 重複拾取，刷新時長。 |
| **滿級地圖道具 (Lv.40)** | 使用高級道具 | B. 效果生效中 | - | - | - | **點擊攔截**：效果已達滿級，不重複扣費。 |

#### 8.2.2 核心規則摘要說明

1. **高級優先級高**：高級道具（滿級屬性）生效中，若撿到地圖道具，Buff 持續時間會被重置為 10 秒，且數值強度**維持滿級**，不會被地圖道具降級。
2. **滿級狀態點擊攔截（防呆）**：只要當前 Buff 強度已是 Lv.40 滿級（不論是透過高級道具觸發，還是玩家被動已升滿 Lv.40 從地圖撿的），高級按鈕會進入 **「B. 效果生效中」半透明置灰 50% 狀態**（見 Ch 8.1.3 狀態表），點擊時直接攔截，不扣費，防止玩家浪費金幣。
3. **時間重置（續杯）**：效果期間內只要重複拾取同種地圖道具，效果持續時間一律**重置為 10 秒最大值**。
...
* **反應延遲**：`AI_REACTION_DELAY = 100ms`（由 [16.5 電腦 AI 策略參數](#165-電腦-ai-策略參數) 控制）。
...
#### 10.1.1 被動屬性強化介面佈局示意圖

```text
+--------------------------------------------------------------+
| [返回]                  ★ 被動屬性強化 ★        金幣: 15,200G |
|--------------------------------------------------------------|
| 1. 磁吸半徑   Lv.15 [███░░]  | 6. 巨大持續   Lv.12 [██░░░]   |
| 2. 巨大速跑   Lv.08 [█░░░░]  | 7. 幸運持續   Lv.02 [░░░░░]   |
| 3. 幸運食物   Lv.20 [███░░]  | 8. 鷹眼持續   Lv.15 [███░░]   |
| 4. 鷹眼視野   Lv.10 [██░░░]  | 9. 岩石抗性   Lv.30 [████░]   |
| 5. 磁鐵持續   Lv.05 [█░░░░]  | 10.河流抗性   Lv.01 [░░░░░]   |
|--------------------------------------------------------------|
| 規則: 點擊【隨機強化】將從未滿級屬性中等機率隨機提升一個等級  |
| 已強化次數: 114 次                  本次隨機強化消耗: 18,340 G|
|                                                              |
|                        [ 隨 機 強 化 ]                       |
+--------------------------------------------------------------+
```
...
貼圖可自里程碑解鎖或從商店購買。氣泡顯示時長固定為 `EMOTE_DURATION = 2500ms`（由 [16.8 個性化外觀與貼圖參數](#168-個性化外觀與貼圖參數) 控制）。

* **擊殺成功** ➔ 蛇頭上方彈出「得意」氣泡（放大抖動 0.5s，靜止，0.5s 漸隱）。
* **被擊殺** ➔ 於死亡碎裂處彈出「大哭」氣泡（緩慢下落並漸隱）。
* **被截斷** ➔ 蛇頭彈出「生氣」氣泡（伴隨紅驚嘆號閃爍並漸隱）。
* **擦身近掠** (頭頭距離 < 60px) ➔ 雙方彈出「求饒」氣泡，快速淡入。
...
#### 13.1.1 外觀商店與更換外觀介面佈局示意圖

```text
+--------------------------------------------------------------+
| [返回]                    ★ 外觀更換與商店 ★    金幣: 15,200G |
|--------------------------------------------------------------|
|    蛇隻外觀預覽     |           外 觀 網 格 (Skins)           |
|                     |                                        |
|     .======.        | +------------+ +------------+ +------+ |
|    ( 0  0   )       | |   經典綠   | |   霓虹光   | | 金黃 | |
|     \  __  /        | |  (已擁有)  | |  (已擁有)  | | 5000| |
...
* **快捷加入流程**：
  ...
  3. 房間碼一次性有效：當隊長按下開始配對並成功進入戰局，或房間閒置超時 `300` 秒後，該房間碼自動失效回收。

### 14.2 隊伍大廳規格

* **隊伍人數限制**：
  * **Solo / 大亂鬥模式**：支援 1~8 人房間，主要作為好友一同社交與語音練習之用。
  * **陣營對抗模式**：限制最大 `4` 人，組成藍隊或紅隊。
* **隊伍大廳 UI 佈局示意圖**：

```text
+--------------------------------------------------------------+
| [返回]          ★ 隊伍大廳 ★        房間碼: 20155 (隊長: Me) |
|--------------------------------------------------------------|
|  [隊長準備]  [邀請好友]  [更換模式]                           |
|                                                              |
|  隊友列表:                                                    |
...
* **隊長權限**：隊長為房主，擁有踢出隊友、更換對局模式（多人大亂鬥 / 陣營對抗）、以及發起配對的唯一權限。
* **隊長離線/移交**：若隊長主動退出或斷線，房主權限會依加入隊伍的先後順序，自動無縫轉移給下一個最先加入的隊友。
* **準備狀態同步**：非隊長成員需手動點擊「準備」按鈕。當所有隊員均顯示為 `[Ready]` 狀態後，隊長方可點擊「開始配對」。

### 14.4 好友與隊友狀態同步

系統對大廳內的其他玩家或隊友頭像邊框進行實時狀態同步，其視覺指示規則如下：

* **在線 (Online)**：頭像外框高亮綠色圓圈。
* **配對中 (Matching)**：頭像外框黃色旋轉粒子圈。
* **對局中 (In-game)**：頭像外框顯示紅色，並在下方以小字顯示對局剩餘時間（如 `1:20`）。
* **離線 (Offline)**：頭像進行灰度（Grayscale）處理，無邊框。
...
### 16.1 參數與數據真源 (Single Source of Truth) 對照表 📝[v13.0]
...
| `<Module表_個性外觀>` | [16.8 個性化外觀與貼圖參數](#168-個性化外觀與貼圖參數) | `EMOTE_DURATION` 表情氣泡在蛇頭上方持續時間。 |
...
| `RATING_BONUS_SSS`   |    50    |  點  | 評級 SSS 額外加成熟練度 📝[v12.0]                          |
| `RATING_BONUS_SS`    |    30    |  點  | 評級 SS 額外加成熟練度 📝[v12.0]                           |
| `RATING_BONUS_S`     |    10    |  點  | 評級 S 額外加成熟練度 📝[v12.0]                            |
...
*Powered by Antigravity v13.0 - 玩星派對專用*
