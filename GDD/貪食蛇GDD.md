...
**文件版本**：29
**更新日期**：2026-09-10
**本次更新**：新增衝刺頭頂特效的局內顯示、外觀商店取得與更換規格。
...
<details open>
<summary><b>v29.0 (2026-09-10)</b></summary>

衝刺頭頂特效與外觀系統規格

* **Ch4.6 衝刺與減速視覺特效規範**：
  * 新增衝刺時的蛇頭上方文字特效；繁體中文顯示「奔跑吧」，英文顯示「Flash」。
* **Ch12.4 衝刺特效更換**：
  * 新增衝刺特效於「更換外觀」中的預覽、套用與使用中狀態規格。
* **Ch13.2 商品內容與貨幣**：
  * 外觀商店新增衝刺特效品項，可購買後於「更換外觀」套用。

</details>

<details>
<summary><b>v28.0 (2026-09-07)</b></summary>

遊戲世界尺寸與地圖 Tile 規格同步

* **Ch6.1 地圖尺寸與 Tile 製作**：
  * 遊戲世界尺寸由 `2048 × 2048` 調整為 `1536 × 1536` 個內部單位。
  * 完整地圖美術尺寸同步調整為 `5760 × 5760px`；單格 Tile 維持 `128 × 128px`，整張地圖改為 `45 × 45 = 2025` 格。
  * 基準相機垂直顯示高度 `288` 個內部單位與美術換算比例 `3.75px／內部單位` 維持不變。
* **Ch16.1 與 GDK**：
  * Module `26–27` 的 `MapWidth／MapHeight` 同步由 `2048` 調整為 `1536`。

</details>

<details>
<summary><b>v27.0 (2026-09-01)</b></summary>

長度積分與自然體型上限規格同步

* **Ch7.3–7.4**：
  * 初始蛇身長度調整為 `10` 節、每增加一節所需積分調整為 `20` 分。
* **Ch4.7 蛇隻體型成長與碰撞半徑**：
  * 每個自然體型階段為 `20` 節、每階段倍率增量為 `2000`，自然體型倍率上限為 `20000`，渲染、碰撞與體型跟隨鏡頭過渡時間為 `500ms`。
  * 移除 `SnakeBodyGrowthScaleMaxThresholdSegment`；自然體型改由 `SnakeBodyGrowthScaleMax` 封頂，不再使用節數門檻參數。
  * 同步重算 Ch4.7.2 公式與 Ch4.7.3／4.7.4 的自然體型、鏡頭、巨大蘑菇與鷹眼驗收數值。
* **Ch16.1 Module**：
  * 移除 Module `83` 的已淘汰門檻參數；其餘 Module ID 維持不重編號。

</details>

<details>
<summary><b>v26.0 (2026-08-25)</b></summary>

鍵盤操作與地圖 Tile 製作規格同步

* **Ch1.2 鍵盤操作**：
  * 新增 `W／A／S／D` 移動、`1` 衝刺、`2` 暴食，以及 `3／4／5／6` 對應道具欄第 1～4 格的鍵盤映射。
  * 鍵盤操作與既有觸控搖桿、HUD 按鈕並存；道具按鍵綁定槽位，不永久綁定特定道具。
* **Ch6.1 地圖尺寸與 Tile 製作**：
  * 遊戲世界尺寸調整為 `2048 × 2048` 個內部單位；基準相機於 `1080px` 畫面高度顯示 `288` 個內部單位，美術換算比例為 `3.75px／內部單位`。
  * 完整地圖美術尺寸為 `7680 × 7680px`；單格 Tile 為 `128 × 128px`，整張地圖共 `60 × 60 = 3600` 格。
  * 明確區分世界物件的美術換算與 UI 螢幕像素；`3.75`、`288` 不新增 Module，且不變更 Ch4.7 鏡頭規則。
* **Ch16.1 與 GDK**：
  * Module `26–27` 的 `MapWidth／MapHeight` 同步由 `2000` 調整為 `2048`。

</details>

<details>
<summary><b>v25.0 (2026-08-19)</b></summary>

食物吸附飛行時間與撞擊扣長說明同步

* **Ch7.2 吸附飛行時間**：
  * 基本吸附 `BaseSuctionTravelTime` 調整為 `450ms`。
  * 地圖磁鐵 `MagnetSuctionTravelTime` 調整為 `400ms`。
* **Ch8.1 高級道具**：
  * 磁鐵滿級吸附飛行時間同步為 `400ms`。
* **Ch16.1 與 GDK**：
  * Module `12–13` 的基礎吃食與磁鐵吸附飛行時間，同步為 `450ms／400ms`。
* **Ch5.1.2 撞擊岩石與邊界回彈**：
  * 扣除計算改以 `[!IMPORTANT]` 提示區塊呈現。
  * 範例改以 `[!NOTE]` 呈現，明確列出碰撞前 `51` 節、`int(25.5) = 25` 與 `51 - 25 = 26` 的推導。

</details>

<details>
<summary><b>v24.0 (2026-08-17)</b></summary>

體型成長、地圖生成與資料表一致性整理

* **Ch4.3 速度計算**：
  * 衝刺、巨大蘑菇與緩速區的速度修正統一採萬分比相加；`DashSpeedFactor` 修正為 `2500`，即衝刺 `+25%`。
* **Ch4.4 暴食吸附**：
  * `GluttonySuctionTravelTime` 正式預設調整為 `350ms`；Dev Setting 僅供測試覆寫，未覆寫時使用 Module 預設值。
* **Ch4.7 蛇隻體型、碰撞與鏡頭**：
  * **體型階段**：
    * 自然體型改為 `50–99／100–149／150–199／200–249／250–299／≥300` 六個區間。
    * `300` 為自然體型 `1.500x` 的倍率門檻，不限制蛇隻繼續成長。
  * **渲染與碰撞**：
    * 渲染尺寸與碰撞半徑共用最終體型倍率。
    * 跨階與巨大蘑菇狀態變化採 `250ms` 過渡，碰撞使用當前插值半徑。
  * **鏡頭與道具交互**：
    * 巨大蘑菇乘入最終體型。
    * 體型跟隨鏡頭依最終體型的 `40%` 計算。
    * 鷹眼再相乘，最終鏡頭上限為 `5.000x`，不限制體型、渲染或碰撞。
  * **參數與驗收**：
    * Module `80–87` 與 GDK 同步，並補入各體型階段、巨大蘑菇與鷹眼組合的驗收表。
* **Ch5.1 衝刺撞蛇身**：
  * 雙方衝刺時，僅主動撞擊方物理回彈；受擊方不回彈，且雙方不死亡、不截斷、不計擊殺。
* **Ch6 地圖生成**：
...
<details>
<summary><b>v23.0 (2026-08-04)</b></summary>
...
**鍵盤操作**：鍵盤輸入與既有觸控搖桿、HUD 按鈕並存，不取代觸控操作。

| 功能 | 按鍵 | 輸入規則 |
| :--- | :---: | :--- |
| 基本移動方向 | `W／A／S／D` | 持續按住以控制蛇隻移動方向。 |
| 衝刺 | `1` | 按住發動，放開停止。 |
| 暴食 | `2` | 按下立即施放。 |
| 道具欄第 1 格 | `3` | 使用 HUD 道具欄由左至右第 1 格的道具。 |
| 道具欄第 2 格 | `4` | 使用 HUD 道具欄由左至右第 2 格的道具。 |
| 道具欄第 3 格 | `5` | 使用 HUD 道具欄由左至右第 3 格的道具。 |
| 道具欄第 4 格 | `6` | 使用 HUD 道具欄由左至右第 4 格的道具。 |

> [!NOTE]
> `3／4／5／6` 固定對應道具槽位，不永久綁定鷹眼、磁鐵、巨大蘑菇或幸運糖等特定道具。
...
| :---------------------------------------- | :------------------------------------------------------------------ | :-------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------- | :--------------------------------------------------------- |
| **教學關卡 1：基本移動** | 小型封閉沙盒（約 1200×1200 px），無敵人、無岩石、無緩速區，場上散落高辨識度食物。 | 教導基本方向操作、吃食物及蛇身成長的因果關係。 | 1. 顯示關卡 1 圖卡。 <br> 2. 左下搖桿區出現手指光暈，引導玩家拖曳並完成轉向。 <br> 3. 高亮前方食物並顯示前往虛線，引導玩家吞食。 <br> 4. 吞食後放慢蛇身增長演出，強調新增蛇節。 | 完成至少 1 次方向轉換、吃到 10 顆食物，並觸發至少 1 次蛇身增長。 | 無。撞牆僅彈回，不致死亦不扣分。 |
...
* **初始生成位置**：單局開始時，玩家與最多 7 隻電腦 AI（共最多 8 隻蛇）的初始生成座標採**隨機選定**。安全選點規則為：生成座標不得與任何岩石障礙物或地圖邊界重疊，允許於平地區或緩速區生成。
...
| :--------------------- | :------: | :-------: | :------------------------------------------- | :------------------------------ |
| **基礎移動速度** | `16000` | 0.01 px/s | 蛇隻無加速時的移動速度（160.00 px/s）        | `BaseMoveSpeed`                  |
| **轉向迴轉半徑** |  `5`  |    px    | 蛇隻轉向時使用的迴轉半徑                     | `SnakeTurnRadius`               |
| **初始蛇身長度** |  `10`  |    節    | 剛進入戰場或重生時的蛇節數量                 | `BaseSnakeSegment`              |
| **體力上限值**   | `1000` |    點    | 衝刺時消耗 the 體力最大池（可連續衝刺 3 秒） | `BaseMaxStamina`                 |
| **體力回復速度** |  `80`  |   點/s   | 停止衝刺後每秒自動恢復的體力量               | `BaseStaminaRecoverRate`         |
| **每節所需積分** |  `20`  |    分    | 蛇身每增加 1 節所需的積分增量                | `PointPerSegment`          |
| **常態吸附半徑** |  `50`  |    px    | 未開啟暴食與磁鐵時的基礎食物吸附半徑         | `BaseSuctionRadius`         |
| **蛇頭碰撞半徑** |  `12`  |    px    | 基礎物理半徑；動態值依 [Ch4.7](#47-蛇隻體型成長與碰撞半徑) 計算 | `SnakeHeadCollisionRadius` |
| **蛇身碰撞半徑** |  `8`  |    px    | 基礎物理半徑；動態值依 [Ch4.7](#47-蛇隻體型成長與碰撞半徑) 計算 | `SnakeBodyCollisionRadius` |

#### 4.3 基礎跑速與速度計算規則

* **基礎跑速**：初始基礎跑速單位為 `BaseMoveSpeed = 16000`（等同於 `160.00 px/s`，即以 100 代表 1 px/s，由 [Ch16.1 Module](#161-module) 控制）。
* **速度修正單位**：衝刺、巨大蘑菇與緩速區的速度修正均採萬分比加算；未生效的效果不計入速度修正總和。
* **速度計算公式**：

> [!IMPORTANT]
>
> ```text
> 最終跑速 = 基礎跑速 × (10000 + 速度修正總和) / 10000
> ```

* **實例計算說明**：

  * 假設基礎跑速為 `16000`。
  * 當前狀態為：衝刺加成 `DashSpeedFactor = +5000`、滿級巨大蘑菇跑速 `ItemMushroomSpeed = +1500`、緩速區修正 `SlowZoneSpeedFactor = -5000`。

> [!IMPORTANT]
>
> ```text
> 速度修正總和 = 5000 + 1500 - 5000 = 1500
> 最終跑速 = 16000 × (10000 + 1500) / 10000 = 18400（等同於 184.00 px/s）
> ```
...
| :-----------------------------------------------: | :------------------------ | :---------------------------------- | :------------------- | :------------------------------------------------------------------------ | :-------------------------------------------------- | :------------------------------------------------ |
| ![衝刺](<./Reference Image/Ref_Icon_主動技-衝刺.png>) | **衝刺 (Dash)**     | [Ch16.1 Module](#161-module) | 無須解鎖 (初始 Lv 0) | **體力上限**：可升至 Lv4  <br> **體力回復速度**：可升至 Lv4 | 每秒消耗 `333` 點體力。加速 `+50%` 移動速度。   | 體力歸 0 進入超載：鎖定禁衝刺與回體，回滿後解鎖。 |
| ![暴食](<./Reference Image/Ref_Icon_主動技-暴食.png>) | **暴食 (Gluttony)** | [Ch16.1 Module](#161-module) | 成長之路 Lv 1 解鎖   | **吸附範圍**：可升至 Lv4  <br> **冷卻時間縮短**：可升至 Lv4 | 瞬間釋放暴食力場，區域內食物於 `350ms` 內飛向蛇頭並完成吞食判定（由 `GluttonySuctionTravelTime` 控制）。Dev Setting 僅供測試覆寫，未覆寫時使用 Module 預設值。 | 釋放後進入冷卻。單局無次數限制。                  |
...
* **吸附範圍升級表**（各等級的食物吸附飛行時間固定為 `350ms`，由 `GluttonySuctionTravelTime` 控制）：
...
蛇身渲染除長度外，需明確定義節點視覺間距、基礎尺寸與動態體型倍率，以避免高速移動時節點過密或過疏。基礎半徑與節點間距如下，體型倍率套用方式統一見 [4.7 蛇隻體型成長與碰撞半徑](#47-蛇隻體型成長與碰撞半徑)。

| 項目 | 建議數值 | 說明 |
| :--- | :---: | :--- |
| **蛇節視覺間距** | `10 px` | 頭部與第一節，以及每節之間的固定螢幕渲染距離；不隨體型倍率增加，體型放大造成相鄰蛇節視覺重疊屬預期效果。 |
| **蛇頭基礎碰撞半徑** | `12 px` | 蛇頭基礎物理半徑，動態值依 [Ch4.7](#47-蛇隻體型成長與碰撞半徑) 計算。 |
| **蛇身基礎碰撞半徑** | `8 px` | 蛇身節點基礎物理半徑，動態值依 [Ch4.7](#47-蛇隻體型成長與碰撞半徑) 計算。 |
| **食物碰撞半徑** | — | 各食物種類的碰撞物理半徑，詳見 [7.1 全域食物規格對照表](#71-全域食物規格對照表)。 |
| **吸附生效距離** | 蛇頭碰撞半徑 + 當前食物碰撞半徑 + 2 px | 判定食物已被吃掉的最小距離閾值；蛇頭半徑採 [Ch4.7](#47-蛇隻體型成長與碰撞半徑) 的當前值。 |
...
* **衝刺加速特效**：
  ...
  * **頭頂文字特效**：衝刺期間在蛇頭上方顯示目前裝備的衝刺文字特效；預設繁體中文顯示「奔跑吧」，英文顯示「Flash」。停止衝刺時立即隱藏。
* **減速狀態特效（跑速低於基礎速度時）**：
...
* 蛇隻依目前蛇身節數跨入體型階段時，提高自然體型倍率；頭部、身體的渲染尺寸與碰撞半徑同步放大。
* 自然成長造成的體型、渲染、碰撞與體型跟隨鏡頭只在蛇身跨入下一個 `20` 節體型階段時更新；`10–29` 節維持初始體型與鏡頭，達自然體型倍率上限後不再放大。
* 巨大蘑菇開始、結束或被覆蓋時，依目前自然體型階段重新計算最終體型，並依 4.7.4 更新渲染、碰撞與鏡頭。
* 玩家蛇與電腦 AI 使用相同計算流程。長度變化依 [Ch7.4 長度成長關係](#74-長度成長關係)，碰撞結果依 [Ch5.1 生死與碰撞判定矩陣](#51-生死與碰撞判定矩陣)。

![蛇隻體型成長示意畫面](<./Reference Image/Ref_體型變大.png>)

##### 4.7.1 計算依據與結果

* 計算以**目前蛇身節數**為變動輸入。
* 初始蛇身節數與頭部／身體基礎碰撞半徑依 [Ch4.2 初始蛇隻基礎屬性規格表](#42-初始蛇隻基礎屬性規格表)。
* 蛇節中心間距依 [Ch4.5 蛇隻視覺節奏與尺寸對照表](#45-蛇隻視覺節奏與尺寸對照表)；基礎視野倍率依 [Ch16.1 Module](#161-module)。

**計算流程**：目前蛇身節數 → 自然體型倍率 → 最終體型倍率（巨大蘑菇）→ 渲染／碰撞尺寸與體型跟隨鏡頭 → 最終鏡頭倍率（鷹眼與上限）。

> 計算結果更新自然體型倍率、最終體型倍率、頭部／身體的渲染尺寸與當前碰撞半徑，以及體型跟隨鏡頭倍率與最終鏡頭倍率。
> 本節新增的可調參數見下表：

**體型參數（Module 80–82、84–87）**

| Module ID | 參數名稱 | 用途 | 預設值 | 單位 |
| :---: | :--- | :--- | ---: | :--- |
| 80 | `SnakeBodyGrowthStageSegment` | 每個自然體型階段的節數。 | `20` | 節 |
| 81 | `SnakeBodyGrowthStageScaleIncrement` | 每個自然體型階段的倍率增量。 | `2000` | 萬分比 |
| 82 | `SnakeBodyGrowthScaleMax` | 自然體型倍率上限。 | `20000` | 萬分比 |
| 84 | `SnakeBodyScaleTransitionTime` | 體型渲染尺寸、碰撞半徑與體型跟隨鏡頭的過渡時間。 | `500` | ms |
| 85 | `SnakeBodyVisionFollowRatio` | 最終體型倍率反映至體型跟隨鏡頭的比例。 | `4000` | 萬分比 |
| 86 | `EagleEyeVisionTransitionTime` | 鷹眼鏡頭倍率的過渡時間。 | `500` | ms |
| 87 | `SnakeFinalVisionScaleMax` | 鷹眼乘算後的最終鏡頭倍率上限。 | `50000` | 萬分比 |

以上體型參數的完整型別與數值見 [Ch16.1 Module](#161-module) 的 ID 80–82、84–87。
...
>
> ```text
> 自然體型倍率 = min(初始體型倍率 10000 + int(max(目前蛇身節數 - 初始蛇身節數 10（BaseSnakeSegment）, 0) / 每階段節數 20（SnakeBodyGrowthStageSegment）) × 每階段自然體型倍率增量 2000（SnakeBodyGrowthStageScaleIncrement）, 自然體型倍率上限 20000（SnakeBodyGrowthScaleMax）)。
> ```
>
> 初始體型倍率為 `10000 = 1.000x`。
> 因此自然體型只會在 `30／50／70／90／110` 節更新為 `1.200x／1.400x／1.600x／1.800x／2.000x`；
> 低於 `30` 節維持 `1.000x`，蛇身達 `110` 節後維持 `2.000x`。
...
>
> ```text
> 當前碰撞半徑 = int(基礎碰撞半徑 × 當前最終體型倍率 / 10000 + 0.5)
> ```
>
> **取整規則**：所有正數倍率與半徑計算採正數四捨五入：`int(x + 0.5)`。

渲染尺寸與碰撞半徑共用同一個當前最終體型倍率。
巨大蘑菇生效時的最終體型倍率見 4.7.4。

**體型更新規則**

* 渲染尺寸與碰撞半徑共用同一個當前插值的最終體型倍率。
* 吃食物、扣長、截斷或重生後，僅在跨越體型階段時重新設定自然體型目標。
* 跨越體型階段時，渲染尺寸、碰撞半徑與體型跟隨鏡頭以 `500ms` 從目前插值值線性過渡至新目標；碰撞判定採用當前插值半徑。
* 過渡期間再次跨越體型階段時，從目前插值值重新過渡至新目標。

##### 4.7.3 體型跟隨鏡頭

鏡頭依目前體型階段的最終體型倍率計算，並在跨入體型階段或巨大蘑菇開始／結束時，以 `500ms` 平滑過渡。
各階段的實際鏡頭倍率見下表。
...
>
> ```text
> 體型跟隨鏡頭倍率 = 基礎視野倍率 10000（BaseVisionScale）+ max(當前最終體型倍率 - 基礎視野倍率 10000（BaseVisionScale）, 0) × 體型跟隨鏡頭比例 4000（SnakeBodyVisionFollowRatio）/ 基礎視野倍率 10000（BaseVisionScale）。
> ```

視野倍率代表鏡頭可視世界範圍倍率；例如 `1.050x` 代表可視世界範圍增加 `5%`，不是 UI 尺寸放大。
當最終體型變化時，體型跟隨鏡頭使用同一個 `500ms` 的當前插值體型逐幀計算，不另行播放獨立鏡頭動畫。

以下為預設值的完整階段驗收結果；鷹眼使用滿級 `1.750x`，巨大蘑菇使用滿級 `1.500x`：

| 節數 | 自然體型 | 自然鏡頭 | 自然＋鷹眼鏡頭 | 蘑菇體型 | 蘑菇鏡頭 | 蘑菇＋鷹眼鏡頭 |
| :--- | ---: | ---: | ---: | ---: | ---: | ---: |
| `10–29` | `1.000x` | `1.000x` | `1.750x` | `1.500x` | `1.200x` | `2.100x` |
| `30–49` | `1.200x` | `1.080x` | `1.890x` | `1.800x` | `1.320x` | `2.310x` |
| `50–69` | `1.400x` | `1.160x` | `2.030x` | `2.100x` | `1.440x` | `2.520x` |
| `70–89` | `1.600x` | `1.240x` | `2.170x` | `2.400x` | `1.560x` | `2.730x` |
| `90–109` | `1.800x` | `1.320x` | `2.310x` | `2.700x` | `1.680x` | `2.940x` |
| `≥110` | `2.000x` | `1.400x` | `2.450x` | `3.000x` | `1.800x` | `3.150x` |

##### 4.7.4 道具交互規則

* **巨大蘑菇**：
  * 自然體型倍率先依 4.7.2 計算，再乘上巨大蘑菇體型倍率。
  * 渲染尺寸、頭部碰撞半徑、身體碰撞半徑與體型跟隨鏡頭皆使用同一個當前最終體型倍率。

> [!IMPORTANT]
>
> ```text
> 最終體型倍率 = 自然體型倍率 × 巨大蘑菇體型倍率 15000（ItemMushroom_Size；未生效時為 10000）/ 10000。
> ```

自然體型倍率最高為 `2.000x`；巨大蘑菇的 `1.500x` 乘算後，最終體型倍率最高為 `3.000x`。
巨大蘑菇開始、結束或被覆蓋時，最終體型、渲染尺寸、碰撞半徑與體型跟隨鏡頭皆以 `500ms` 從當前插值值線性過渡至新目標。

* **鷹眼**：
  * 鷹眼套用在體型跟隨鏡頭倍率上，而非基礎視野。
  * 未使用鷹眼時，`ItemEyeScale` 視為 `10000 = 1.000x`。
  * 最終鏡頭倍率再套用上限。

> [!IMPORTANT]
>
> ```text
> 最終鏡頭倍率 = min(體型跟隨鏡頭倍率 × 鷹眼視野倍率 17500（ItemEyeScale；未生效時為 10000）/ 基礎視野倍率 10000（BaseVisionScale）, max(最終鏡頭倍率上限 50000（SnakeFinalVisionScaleMax）, 基礎視野倍率 10000（BaseVisionScale）))。
> ```

「最終鏡頭倍率上限 `SnakeFinalVisionScaleMax`」低於「基礎視野倍率`BaseVisionScale`」時，以 `BaseVisionScale` 生效；超過上限時，鏡頭停在上限。
體型、渲染尺寸與碰撞半徑仍依最終體型倍率計算，不受鏡頭回寫。
鷹眼開始、結束或被覆蓋時，鷹眼倍率以 `500ms` 從當前插值值線性過渡至新目標。

> [!NOTE]
>
> 蛇身達 `110` 節以上時：無道具的體型跟隨鏡頭為 `1.400x`；僅鷹眼為 `2.450x`；僅巨大蘑菇的最終體型為 `3.000x`、體型跟隨鏡頭為 `1.800x`；巨大蘑菇加鷹眼的最終鏡頭為 `3.150x`。

**體型與鏡頭更新規則**

* 吃食物、扣長、截斷或重生時，僅於跨越體型階段時重新計算自然體型倍率；巨大蘑菇開始、結束或被覆蓋時，直接依目前體型階段重新計算最終體型倍率。
* 跨越體型階段或巨大蘑菇狀態變化時，渲染、碰撞與體型跟隨鏡頭在同一事件、同一幀開始 `500ms` 過渡；過渡期間再次變化時，從當前插值值重新設定目標。
* 鷹眼開始、結束或被覆蓋時，只以 `500ms` 過渡鷹眼倍率；每一幀以當前體型跟隨鏡頭倍率乘上當前鷹眼插值倍率，再套用最終鏡頭倍率上限。
...
局內生死邏輯基本原則為「撞身即死」；衝刺撞擊例外依下列判定矩陣處理。
頭對頭碰撞或衝刺截斷時執行以下判定矩陣。
...
graph TD
    ...
    D -->|否| E["攻擊方死亡，受擊方擊殺 +1"]
    D -->|是| F{"受擊方是否衝刺?"}
    F -->|是| G["僅攻擊方物理回彈，不致死、不截斷"]
    F -->|否| H{"撞擊位置是否 <= 10 節?"}
    H -->|是| I["受擊方死亡，攻擊方擊殺 +1"]
    H -->|否| J["受擊方從撞擊點截斷，生成殘骸"]
    B -->|敵方蛇頭| K{"雙方衝刺狀態"}
    K -->|雙方未衝刺| L["雙方死亡，各自擊殺 +1"]
    K -->|攻擊方衝刺，受擊方未衝刺| M["受擊方死亡，攻擊方擊殺 +1"]
    K -->|雙方衝刺| N["雙方物理回彈，不致死"]
    E --> O["死亡碎裂與殘骸生成"]
    I --> O["死亡碎裂與殘骸生成"]
    J --> O
...
| :----: | :--------: | :---------: | :--------: | :--------: | :--------: | :--------: | :--------: | :------------------------------------- |
|   頭   |     身     |   未衝刺   |    任意    |     O     |     X     |     -     |     +1     | 攻擊方碎裂死亡，原地生成殘骸。         |
|   頭   |     身     |    衝刺    |   未衝刺   |     X     |     X     |     -     |     -     | 依撞擊位置：前 10 節擊殺；其餘從撞擊點截斷。 |
|   頭   |     身     |    衝刺    |    衝刺    |     X     |     X     |     -     |     -     | 僅攻擊方物理回彈；受擊方不回彈，不致死、不截斷。 |
...
攻擊方衝刺且受擊方未衝刺時，依撞擊位置節數觸發不同死傷判定：

* **雙方衝刺例外**：攻擊方蛇頭撞到受擊方蛇身時，僅攻擊方物理回彈；受擊方不回彈，雙方不死亡、不截斷、不計擊殺，且不進入以下撞擊位置判定。

* 撞擊位置 ＜＝ 10 節 (頭部區) ➔ 受擊方死亡。**攻擊方擊殺數 +1**。
* 撞擊位置 ＞ 10 節 (身體區) ➔ 受擊方被截斷；被切掉的身體節數會立即移除，最短只會剩下頭部區。
* **截斷殘骸**：依被切掉的身體節數計算，沿被切掉的蛇身平均散落；這些節數已移除，後續死亡時不得重複計算。

> [!IMPORTANT]
>
> ```text
> 截斷殘骸顆數 = int(被切掉的身體節數 * 50%)
> 截斷殘骸總積分 = 截斷殘骸顆數 * 10
> ```
...
>
> ```text
> 應扣節數 = 目前蛇身節數 × ImpactBoundaryLengthDeductionRatio / 10000
> ```
>
> 實際扣除節數採 `int(應扣節數)`，正數小數無條件捨去。既有「最少保留頭部區」下限優先適用。

> [!NOTE]
>
> **例**：碰撞前目前蛇身為 `51` 節，且 `ImpactBoundaryLengthDeductionRatio = 5000`：
>
> ```text
> 應扣節數 = 51 × 5000 / 10000 = 25.5
> 實際扣除節數 = int(25.5) = 25
> 扣除後剩餘蛇身節數 = 51 - 25 = 26
> ```
...
>
> ```text
> 死亡殘骸顆數 = int(死亡當下目前蛇身節數 * 50%)
> 死亡殘骸總積分 = 死亡殘骸顆數 * 10
> ```

> [!NOTE]
> 例：死亡當下為 290 節，則 `int(290 * 50%) = 145` 顆殘骸結晶，殘骸總積分為 `145 * 10 = 1450` 分。`int()` 後的小數殘值直接捨去，不保留或累積至後續事件。

* **積分判定**：吃下由殘骸轉化的食物，會增加本次生命週期的長度成長進度，也會增加累計吞食積分；殘骸食物在未被吃掉前，不列入地圖常態食物補充的檢測計算基數。
...
* **遊戲世界尺寸**：寬度 `MapWidth = 1536`、高度 `MapHeight = 1536`（由 [Ch16.1 Module](#161-module) 控制）。兩者皆為遊戲世界的內部座標，沿用既有世界規格的 `px` 欄位語意，不代表螢幕實際像素數。
* **基準相機與換算來源**：當 `BaseVisionScale = 10000（1.000x）`、畫面高度為 `1080px` 時，相機垂直顯示 `288` 個內部單位。`288` 由蛇頭碰撞半徑 `SnakeHeadCollisionRadius = 12` 推導：蛇頭直徑為 `24` 個內部單位，相機基準高度等於 `12` 個蛇頭直徑。

> [!IMPORTANT]
>
> ```text
> 蛇頭直徑 = 蛇頭碰撞半徑 12 × 2 = 24 個內部單位
> 基準相機垂直顯示高度 = 蛇頭直徑 24 × 12 = 288 個內部單位
> 美術換算比例 = 畫面高度 1080px ÷ 288 個內部單位 = 3.75px／內部單位
> 完整地圖美術單邊尺寸 = 1536 個內部單位 × 3.75 = 5760px
> 單邊 Tile 格數 = 5760px ÷ 128px = 45 格
> 完整地圖 Tile 格數 = 45 × 45 = 2025 格
> ```

| 製作項目 | 規格 | 說明 |
| :--- | :---: | :--- |
| 遊戲世界尺寸 | `1536 × 1536` | 遊戲內部座標，不是螢幕像素。 |
| 完整地圖美術尺寸 | `5760 × 5760px` | 依 `1536 × 3.75` 換算。 |
| 單格 Tile 美術尺寸 | `128 × 128px` | 由 `256 ÷ 2 = 128` 取得。 |
| 地圖 Tile 格數 | `45 × 45` | 共 `2025` 格。 |

* **世界物件美術換算**：地圖、地形、蛇、食物與世界場景物件的美術製作尺寸，以遊戲內部單位乘以 `3.75` 換算為美術像素。
* **UI 不套用換算**：搖桿、按鈕、排行榜與 Buff 圖示等 UI 維持螢幕像素 `1:1`，不乘以 `3.75`。
* **鏡頭與參數邊界**：`3.75` 是美術資產製作比例；Ch4.7 的鏡頭拉遠仍縮放整個世界畫面，不改寫資產比例。`3.75` 與 `288` 不新增 Module，也不修改 Ch4.7 鏡頭參數。
* **Tile 適用範圍**：Tile 規則只定義地圖美術拼接，不改動岩石、緩速區、食物與地圖道具既有的生成或碰撞邏輯。

> [!NOTE]
> 下圖為地圖 Tile 拼接、地形與場景物件的美術製作示意；正式配置仍依本節數值與各物件既有規則執行。

![地圖 Tile 製作示意](<./Reference Image/Ref_Map_Tile製作示意.png>)
...
| :--- | :---: | :--- |
| **基本吸附** | `450ms` | `BaseSuctionTravelTime` |
| **地圖磁鐵** | `400ms` | `MagnetSuctionTravelTime` |
| **暴食** | `350ms` | `GluttonySuctionTravelTime` |
...
* **本次生命週期長度成長進度（物理成長線）**：
  * 吃食物後，食物分數會先累積在長度成長進度中；每湊滿「每節所需積分」`PointPerSegment = 20` 分（由 [Ch16.1 Module](#161-module) 控制），蛇身增加 1 節。
...
* **初始長度與重生重置**：新局開始時蛇身長度為 `BaseSnakeSegment = 10` 節（由 [Ch16.1 Module](#161-module) 控制）；玩家重生後，蛇身重置為此初始長度。
* **重生不加分**：重生只重置蛇身長度，不會額外增加對局分數或累計吞食積分。
* **長度成長規則**：吃到新食物時，食物分數累積至本次生命週期的長度成長進度；每累積 20 分，蛇身增加 1 節。
...
| :--- | :--- | :--- | :--- | :--- |
| ![磁鐵](<./Reference Image/Ref_Icon_道具-磁鐵.png>) | **磁鐵 <br> (Magnet)** | 啟動強制大範圍引力，自動吸附周圍食物向蛇頭飛行。 | 吸附半徑：`ItemMagnetRadius = 200px` <br> 吸附飛行時間：`MagnetSuctionTravelTime = 400ms` | 20s <br> (由 `ItemMagnetDuration` 控制) |
...
| :---------------------------------------------------------- | :--------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------: |
| **A. 可使用狀態**  <br> *(次數>0 且 道具未生效)*  | 按鈕正常高亮，顯示剩餘次數。 <br> 下方顯示當前使用所需金幣。               | 1. 成功扣除當前需消耗的金幣數。 <br> 2. 該道具單局剩餘次數 **`-1`**。 <br> 3. 即刻發動道具滿級效果（持續 20 秒）。 <br> 4. 更新下次使用之等差消耗售價。 | **無**  <br> *(直接觸發使用與特效)* |
...
* **入口與分類**：玩家於「更換外觀」介面新增「衝刺特效」頁籤；此頁籤獨立於造型與表情貼圖設定。
* **顯示與套用**：
  * 展示所有已解鎖與未解鎖的衝刺特效；特效卡須提供蛇頭上方文字的預覽，並依目前遊戲語言顯示對應文字。
  * 點擊【已擁有】的衝刺特效可選取並套用，卡片顯示「使用中」標籤。
  * 點擊【未擁有】的衝刺特效僅可預覽，不可套用；套用按鈕維持不可用。
* **預設特效**：預設衝刺特效於繁體中文顯示「奔跑吧」、英文顯示「Flash」；局內僅於玩家衝刺期間顯示於蛇頭上方。
...
* 玩家可從遊戲大廳點擊「外觀商店」進入造型、表情貼圖與衝刺特效取得介面。
...
* **衝刺特效取得方式**：衝刺特效可於外觀商店購買；購買後列入「更換外觀」的衝刺特效頁籤供玩家選取與套用。價格、貨幣類型與資源 ID 依後續商品資料配置，未定義前不預設數值。
* **資源購買範例**：
  * **金幣直購 Skin**：部分常規醜萌 Skin 可消耗金幣直接解鎖。
  * **鑽石直購 Skin**：稀有/搞怪 Skin 需消耗鑽石購買。
  * **表情貼圖直購**：未解鎖表情可消耗鑽石直接解鎖。
  * **衝刺特效直購**：未解鎖衝刺特效可依商品資料指定的貨幣購買；購買後可在「更換外觀」中切換。
* **商品狀態標示**：
...
|  | 1 | core | 基礎跑速單位，16000 代表 160.00 px/s | BaseMoveSpeed | 16000 | 0.01 px/s |
...
|  | 7 | core | 開局與重生初始長度 | BaseSnakeSegment | 10 | 節 |
|  | 8 | core | 衝刺加速比例 | DashSpeedFactor | 5000 | 萬分比 |
...
|  | 14 | core | 每增加一節所需積分 | PointPerSegment | 20 | 分 |
...
|  | 80 | core | 每個自然體型階段的節數 | SnakeBodyGrowthStageSegment | 20 | 節 |
|  | 81 | core | 每個自然體型階段的倍率增量 | SnakeBodyGrowthStageScaleIncrement | 2000 | 萬分比 |
|  | 82 | core | 蛇身依長度自然成長時可達的最大體型倍率 | SnakeBodyGrowthScaleMax | 20000 | 萬分比 |
|  | 84 | core | 蛇隻渲染尺寸、碰撞半徑與體型跟隨鏡頭由舊值過渡至新值的線性時間 | SnakeBodyScaleTransitionTime | 500 | ms |
...
|  | 97 | mode | 單局陪練蛇數量上限 | CompanionSnakeMaxCount | 7 | 隻 |
|  | 98 | core | 蛇隻轉向迴轉半徑 | SnakeTurnRadius | 5 | px |
