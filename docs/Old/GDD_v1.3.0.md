...
## 7. 異常處理與邊界條件 (Technical Rules)

### 7.1 網路與斷線處理
- **斷線託管**：玩家斷線後，蛇進入 AI 託管狀態 10 秒（維持原方向直行）。
- **消散邏輯**：若 10 秒內未重連，該蛇立即死亡並將其長度化為能量球。
- **延遲補償**：系統採用「伺服器端判定」為準。前端進行平滑座標預測，當本地判定與伺服器不符時，以伺服器傳回的座標進行位置修正。

---

## 8. 局外獎勵與商業化 (Meta-Progression)
