...
（系統需要補入電腦席位時，根據「補位機率」隨機選用其中一種 AI。玩家斷線不進入重連或 AI 接管流程，依 [15.2 中離懲罰](#152-中離懲罰) 處理。）
...
* **介面互動與彈窗規則**：
  ...
  * **被動屬性 [Icon] 點擊互動**：
    ...
    * **點擊觸發**：玩家點擊任意屬性 `[Icon]` 時，彈出效果詳情 Popup 彈窗，展示該屬性的目前等級效果、下一級效果（若未滿級）以及詳細作用說明（詳見下方 [10.2 被動屬性強化明細](#102-被動屬性強化明細)）。例如點擊「磁吸半徑」的 Icon，彈出：
      ...
      > *說明：提升局內拾取磁鐵道具後的吸附範圍。*
  * **金幣不足狀態**：若玩家持有金幣低於本次隨機強化消耗，`隨機強化` 按鈕維持可見但顯示灰階，消耗金幣文字改為紅色；點擊時不扣費、不抽選屬性，中央顯示漂浮提示 `金幣不足！`。
  * **全滿級狀態**：當 10 種被動屬性皆達 Lv 40，`隨機強化` 按鈕顯示灰階並改文案為 `已全部滿級`；點擊時不觸發任何抽選，中央顯示漂浮提示 `所有被動屬性已滿級`。
...
* **商品狀態標示**：
  ...
  * **貨幣不足**：購買按鈕灰階，價格文字以紅色或不足提示標示；點擊時顯示中央漂浮提示 `貨幣不足`。
...
* **不做斷線重連**：本遊戲不設計斷線重連、狀態快照追幀或 AI 代管回復流程。玩家於對局中關閉頁面、網路中斷、App 被系統關閉或主動離開，均直接視為中離。
* **中離局內處理**：中離成立時，該蛇隻在局內立即暴斃死亡，屍體依 [5.4 死亡碎裂與殘骸生成規格](#54-死亡碎裂與殘骸生成規格) 轉化為食物殘骸。其他玩家不需要看到斷線圖示或重連倒數。
* **中離後續流程**：中離玩家回到流程時直接進入平台結算，不進入一般對局結算畫面；平台結算顯示「中途離開」狀態與折減後獎勵。
...
* **平台結算顯示**：平台結算需顯示 `中途離開`、`最低評價獎勵 50%` 與實際獲得金幣，並明確標示該局不計入正常對局評價展示。
...
| Key | 繁中 | English | 使用位置 / 觸發情境 |
| :--- | :--- | :--- | :--- |
| `ui_lobby_btn_solo` | 單人練習 | Solo Practice | 大廳模式卡片 |
| `ui_lobby_btn_brawl` | 多人大亂鬥 | Brawl | 大廳模式卡片 |
| `ui_lobby_btn_team` | 陣營對抗 | Team Battle | 大廳模式卡片、好友房唯一可選模式 |
| `ui_lobby_btn_tutorial` | 教學模式 | Tutorial | 大廳入口 |
| `ui_lobby_btn_create_room` | 創建好友房 | Create Room | 大廳社交按鈕 |
| `ui_lobby_btn_join_room` | 加入好友房 | Join Room | 大廳社交按鈕 |
| `ui_matchmaking_title` | 配對中 | Matchmaking | 配對中畫面標題 |
| `ui_matchmaking_cancel` | 取消配對 | Cancel | 配對中畫面按鈕 |
| `ui_matchmaking_team_mode` | 陣營對抗 | Team Battle | 好友房配對中模式顯示 |
| `ui_exit_popup_title` | 離開對局？ | Leave Match? | 局中退出二次確認標題 |
| `ui_exit_popup_desc` | 僅獲得最低評價50%獎勵 | Only receive 50% of the lowest rating reward | 局中退出二次確認內容 |
| `ui_common_yes` | 是 | Yes | 通用確認按鈕 |
| `ui_common_no` | 否 | No | 通用取消按鈕 |
| `ui_common_back` | 返回 | Back | 通用返回按鈕 |
| `ui_common_close` | 關閉 | Close | 通用關閉按鈕 |
| `ui_result_rank_retry` | 再接再厲 | Better Luck Next Time | 中離平台結算名次欄 |
| `ui_result_leave_status` | 中途離開 | Left Early | 平台結算狀態 |
| `ui_result_leave_reward` | 最低評價獎勵 50% | 50% Lowest Rating Reward | 中離平台結算獎勵說明 |
| `ui_result_coin_reward` | 本局可獲得金幣 | Coins Earned | 對局結算 / 平台結算 |
| `ui_result_no_drop` | 未獲得隨機掉落獎勵 | No random drop reward | 平台結算掉落區 |
| `ui_room_code_title` | 輸入房間碼 | Enter Room Code | 加入好友房輸入標題 |
| `ui_room_code_invalid` | 房間碼無效或已逾時 | Room code is invalid or expired | 加入好友房錯誤提示 |
| `ui_room_ready` | 準備 | Ready | 隊員準備按鈕 |
| `ui_room_unready` | 取消準備 | Cancel Ready | 隊員取消準備按鈕 |
| `ui_room_start` | 開始配對 | Start Matchmaking | 隊長開始按鈕 |
| `ui_room_start_disabled` | 至少需要 2 人才能開始 | At least 2 players are required | 好友房 1 人時開始按鈕提示 |
| `ui_room_kicked` | 已離開隊伍 | Removed from room | 被隊長踢出或離開隊伍提示 |
| `ui_tip_energy_not_enough` | 能量不足 | Not enough energy | 能量不足共用提示 |
| `ui_tip_coin_not_enough` | 金幣不足！ | Not enough coins! | 高級道具 / 被動強化提示 |
| `ui_tip_effect_active` | 效果生效中！ | Effect is active! | 高級道具效果期間點擊提示 |
| `ui_tip_limit_reached` | 已達單局使用上限！ | Match use limit reached! | 高級道具次數耗盡提示 |
| `ui_passive_info_title` | 被動強化規則說明 | Passive Upgrade Rules | 被動強化 `[i]` Popup |
| `ui_passive_maxed` | 所有被動屬性已滿級 | All passive attributes are maxed | 被動全滿級提示 |
| `ui_passive_btn_maxed` | 已全部滿級 | All Maxed | 被動強化按鈕滿級狀態 |
| `ui_growth_next_preview` | 下一級預覽 | Next Level Preview | 成長之路節點資訊 |
| `ui_growth_claimable` | 可領取 | Claimable | 成長之路手動領取狀態 |
| `ui_growth_active` | 已生效 | Active | 成長之路已生效狀態 |
| `ui_shop_owned` | 已擁有 | Owned | 外觀商店商品狀態 |
| `ui_shop_equipped` | 使用中 | Equipped | 更換外觀目前套用狀態 |
| `ui_shop_currency_lack` | 貨幣不足 | Not enough currency | 外觀商店不足提示 |
| `ui_shop_limited` | 限時 | Limited | 外觀商店限時商品標籤 |
...
*Powered by Antigravity v15.0 - 玩星派對專用*
