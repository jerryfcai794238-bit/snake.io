...
|  D  | 100 - 299      |   1800   |       0       |
|  E  | 99 以下        |   1600   |       0       |
> 來源讀取：`<RatingReward表>`。評級門檻、金幣獎勵與評級額外熟練度皆以此表為唯一真源。
...
> 關卡設定來源讀取：`<TutorialLevel表>`；圖卡步驟順序與文字 Key 來源讀取：`<TutorialStep表>`。
...
* **安全投放與防穿幫重疊**：食物與道具生成時，必須確保其碰撞半徑完全不與岩石或地圖邊界接觸（即物理無碰撞邊緣點）。
  ...
  * 食物與食物、食物與地圖道具、地圖道具與地圖道具之間**允許重疊，但不可完全等同**（即兩者的中心點座標 $X, Y$ 不能完全重合，需由最小間距 `Object_SPAWN_MIN_DIST = 2px` 進行安全隨機偏移控制；來源讀取：`<Module表>`）。
...
* **重疊容許規則**：食物與地圖道具可以視覺上接近或部分重疊，但中心點不可完全相同，需套用 `Object_SPAWN_MIN_DIST = 2px` 的最小安全偏移（來源讀取：`<Module表>`）。
...
| `<PassiveSkill表>` | `string` | [16.6 局內道具直購參數](#166-局內道具直購參數) | 磁鐵、巨大蘑菇、幸運糖、鷹眼的效果數值與持續時間。 |
| `<RatingReward表>` | `string` | [3.4.4 單局結算獎勵與評級對照表](#344-單局結算獎勵與評級對照表) | SSS～E 評級門檻、金幣與熟練度。 |
| `<TutorialLevel表>`／`<TutorialStep表>` | `string` | [3.5 新手教學模式與首次使用者體驗](#35-新手教學模式與首次使用者體驗) | 教學關卡設定、完成條件、圖卡步驟與文字 Key。 |
| `<Food表>` | `string` | [7.3 積分與加分公式](#73-積分與加分公式) | 食物與殘骸的分數、碰撞半徑與渲染尺寸。 |
| `<ModeBotComposition表>` | `string` | [9.1.1 電腦 BOT 對局固定補位配比表](#911-電腦-bot-對局固定補位配比表) | 各模式 BOT 固定補位數量。 |
| `<StringTable表>` | `string` | [17. 字串表與多語系文字參數](#17-字串表與多語系文字參數) | UI 與多語系文字。 |
...
| `SNAKE_HEAD_COLLISION_RADIUS` | `int` | 12 | px | 蛇頭碰撞半徑 |
| `SNAKE_BODY_COLLISION_RADIUS` | `int` | 8 | px | 蛇身碰撞半徑 |
| `SNAKE_SECTION_SPACING` | `int` | 10 | px | 蛇節視覺間距 |
| `CUT_HEAD_SAFE_SECTIONS` | `int` | 10 | 節 | 截斷判定避開蛇頭前段節數 |
...
| `RANKING_DISPLAY_COUNT` | `int` | 4 | 行 | 局內排行榜顯示行數 |
| `TIME_WARNING_THRESHOLD` | `int` | 30 | s | 倒數警示門檻 |
| `EARLY_LEAVE_REWARD_RATIO` | `int` | 5000 | 萬分比 | 中離最低評價獎勵比例 |
| `ROOM_CODE_DIGITS` | `int` | 5 | 位 | 好友房房間碼位數 |
| `ROOM_MIN_PLAYERS` | `int` | 2 | 人 | 好友房最少開始人數 |
| `ROOM_MAX_PLAYERS` | `int` | 4 | 人 | 好友房隊伍人數上限 |
| `PHYSICS_MAX_DT` | `int` | 100 | ms | 單次物理更新最大 dt |
...
| `Object_SPAWN_MIN_DIST` | `int` | 2 | px | 地圖物件生成最小安全偏移 |
| `DEBRIS_SCORE_ROUND_UNIT` | `int` | 10 | 分 | 殘骸分數捨去倍數 |
...
> 道具效果數值來源讀取：`<PassiveSkill表>`；購買價格、刷新與單局使用限制來源讀取：`<Module表>`。
...
| `ITEM_MUSHROOM_SPEED`         | `int` |  -2500  | 萬分比 | 巨大蘑菇速度修正                                              |
| `ITEM_CANDY_DURATION`         | `int` |    10    |   s   | 地圖幸運糖持續時間                                            |
...
> 效果初始值與最高值來源讀取：`<PassiveSkill表>`；中間等級由既有公式計算，不建立逐級資料欄位。費用曲線常數來源讀取：`<Module表>`。

| 參數名稱 | 資料型態 | 初始（Lv 0） | 最高（Lv 40） | 單位 | 參數描述 |
| :--- | :--- | :---: | :---: | :---: | :--- |
| `ITEM_MAGNET_RADIUS` | `int` | 100 | 200 | px | 磁鐵吸附半徑 |
| `ITEM_MUSHROOM_SPEED` | `int` | -2500 | 1500 | 萬分比 | 巨大蘑菇速度修正 |
| `ITEM_CANDY_MULTIPLIER` | `int` | 30000 | 70000 | 萬分比 | 幸運糖得分倍率 |
| `ITEM_EYE_SCALE` | `int` | 13500 | 17500 | 萬分比 | 鷹眼視野倍率 |
| `ITEM_MAGNET_DURATION` | `int` | 10 | 20 | s | 磁鐵持續時間 |
| `ITEM_MUSHROOM_DURATION` | `int` | 10 | 20 | s | 巨大蘑菇持續時間 |
| `ITEM_CANDY_DURATION` | `int` | 10 | 20 | s | 幸運糖持續時間 |
| `ITEM_EYE_DURATION` | `int` | 10 | 20 | s | 鷹眼持續時間 |
| `PASSIVE_ROCK_RESIST` | `int` | -5000 | -1000 | 萬分比 | 岩石扣長減免 |
| `PASSIVE_RIVER_RESIST` | `int` | -5000 | -1000 | 萬分比 | 河流減速減免 |
| `PASSIVE_UPGRADE_START_COST` | `int` | 1000 | - | 金幣 | 強化初始費用 |
| `PASSIVE_UPGRADE_END_COST` | `int` | 300000 | - | 金幣 | 強化最高費用 |
| `PASSIVE_UPGRADE_SHAPE_K` | `int` | 24 | - | 千分比 | S 曲線係數 |
| `PASSIVE_UPGRADE_INFLECTION_X0` | `int` | 190 | - | 次 | S 曲線拐點 |
...
本章收斂遊戲內需要由程式、UI 或營運配置的文字字串。實作時以 `<StringTable表>` 為唯一真源，Key 使用英文固定命名，文字內容提供繁中與英文兩版。
