...
![局中 HUD 參考圖](<Reference Image/Ref_局中.jpg>)

#### 2.2.4 遊戲結算畫面佈局示意圖

![對局結算畫面參考圖](<Reference Image/Ref_對局結算.jpg>)
...
![第一名方位指示參考圖](<Reference Image/Ref_第一名方位指示.jpg>)
...
graph TD
    ...
    I -->|否| H
    I -->|是| J{隊伍人數是否達 2 人}
    J -->|否，僅 1 人| K[開始配對按鈕不可用]
    J -->|是，2 至 4 人| L[隊長開始配對]
    L --> M{隊伍是否滿 4 人}
    M -->|未滿 4 人| N[友方空缺由其他玩家或電腦補足]
    M -->|已滿 4 人| O[原隊伍 4 人進入同一陣營]
    N --> P[進入陣營對抗對局]
    O --> P
    P --> Q[對局結束]
    Q --> R[原好友隊伍返回自己的隊伍大廳]
    Q --> S[補位玩家返回各自隊伍大廳或個人流程]
```
...
* **隊伍人數限制**：
  ...
  * **開始配對人數門檻**：隊伍至少需 `2` 人才可由隊長開始配對；僅 `1` 人在隊伍大廳時，開始配對按鈕不可用或點擊時攔截提示。
...
* **開始配對條件**：隊長僅能在隊伍人數達 `2 ~ 4` 人，且所有非隊長成員均已準備完成時發起配對；隊伍僅 `1` 人時不可開始。
* **隊長離線/移交**：若隊長主動退出或斷線，房主權限會依加入隊伍的先後順序，自動無縫轉移給下一個最先加入的隊友。
* **準備狀態同步**：非隊長成員需手動點擊「準備」按鈕。當所有非隊長隊員均顯示為 `[Ready]` 且隊伍人數達開始門檻後，隊長方可點擊「開始配對」。
