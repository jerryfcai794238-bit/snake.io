...
| **教學關卡 6：高級道具 - 幸運糖**   | 沙盒地圖，散落少量基礎食物。                                        | 引導玩家主動點擊 HUD 中的**幸運糖道具按鈕**，體驗幸運時間內的食物增長與點數加成。 | 1. 畫面下方 HUD 的「幸運糖道具按鈕」高亮閃爍，指引點擊。`<br>`2. 點擊使用後，蛇隻進入持續 10 秒的幸運狀態，食物點數獲取倍率為 3 倍。                                                                                                                                                                                              | 在幸運糖持續時間內吃到至少 25 顆食物，個人長度積分達 300 分。         | 無。                                                       |
...
| :-----------------------------------------------: | :------------------------ | :---------------------------------- | :------------------- | :------------------------------------------------------------------------ | :-------------------------------------------------- | :------------------------------------------------ |
| ![衝刺](<./Reference Image/Ref_Icon_主動技-衝刺.png>) | **衝刺 (Dash)**     | [16.2 基礎核心參數](#162-基礎核心參數) | 無須解鎖 (初始 Lv 0) | **體力上限**：可升至 Lv4 `<br>`**體力回復速度**：可升至 Lv4 | 每秒消耗 `333` 點體力。加速 `+25%` 移動速度。   | 體力歸 0 進入超載：鎖定禁衝刺與回體，回滿後解鎖。 |
| ![暴食](<./Reference Image/Ref_Icon_主動技-暴食.png>) | **暴食 (Gluttony)** | [16.2 基礎核心參數](#162-基礎核心參數) | 成長之路 Lv 1 解鎖   | **吸附範圍**：可升至 Lv4 `<br>`**冷卻時間縮短**：可升至 Lv4 | 瞬間釋放暴食力場，以速度 `12 px/s` 吸附區域食物。 | 釋放後進入冷卻。單局無次數限制。                  |
...
* **同時存在上限**：地圖上最多同時存在 `ITEM_MAX_COUNT = 12` 個地圖道具（由 [16.6 局內道具直購參數](#166-局內道具直購參數) 控制）；若已達上限，該次刷新跳過。
...
* **投放預告**：生成前有 `ITEM_TEASER_TIME = 2s` 的閃爍預告（由 [16.6 局內道具直購參數](#166-局內道具直購參數) 控制）。
...
| :---------------------- | :------- | :-------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------- |
| `<Module表_基礎屬性>` | `VARCHAR(64)` | [16.2 基礎核心參數](#162-基礎核心參數)                 | `BASE_SPEED`、`STAMINA_MAX`、`STAMINA_REGEN_SPEED`、`BASE_SUCTION_RADIUS`、`INITIAL_LENGTH`、`POINTS_PER_SECTION` 等。 |
| `<Module表_地圖場景>` | `VARCHAR(64)` | [16.4 地圖與場景參數](#164-地圖與場景參數)             | `MAP_WIDTH`、`MAP_HEIGHT`、`RIVER_SLOW_MULTIPLIER`、河流與岩石分配權重等。                                                   |
| `<Module表_AI策略>`   | `VARCHAR(64)` | [16.5 電腦 AI 策略參數](#165-電腦-ai-策略參數)         | `AI_REACTION_DELAY`、`AI_STEERING_PRECISION`、`AI_DASH_PROB` 等行為變數。                                                    |
| `<Module表_遊戲模式>` | `VARCHAR(64)` | [16.3 遊戲模式參數](#163-遊戲模式參數)                 | `GAME_DURATION`、`RESPAWN_TIME`、`GHOST_TIME`、門票能量等規則變數。                                                          |
| `<Module表_個性外觀>` | `VARCHAR(64)` | [16.8 個性化外觀與貼圖參數](#168-個性化外觀與貼圖參數) | `EMOTE_DURATION` 表情貼圖在蛇頭上方持續時間。                                                                                    |
...
| `GROWTH_ROAD_MAX_LEVEL`  | `TINYINT UNSIGNED` |    60    |   -   | 成長之路最高上限等級                                    |

#### 16.3 遊戲模式參數

| 參數名稱               | 資料型態 | 預設數值 | 單位 | 參數描述                         |
| :--------------------- | :------- | :------: | :--: | :------------------------------- |
| `GAME_DURATION`      | `INT UNSIGNED` |   120   |  s  | 單局限時時長                     |
| `RESPAWN_TIME`       | `INT UNSIGNED` |   5000   |  ms  | 死亡復活倒數時間                 |
| `GHOST_TIME`         | `INT UNSIGNED` |   2000   |  ms  | 復活幽靈無敵時間                 |
| `ENTRY_ENERGY_SOLO`  | `SMALLINT UNSIGNED` |    30    |  點  | Solo 模式入場參賽能量門票        |
| `ENTRY_ENERGY_BRAWL` | `SMALLINT UNSIGNED` |    30    |  點  | 多人大亂鬥入場參賽能量門票       |
| `ENTRY_ENERGY_TEAM`  | `SMALLINT UNSIGNED` |    30    |  點  | 陣營對抗入場參賽能量門票         |
| `KILL_SCORE`         | `INT UNSIGNED` |   100   |  分  | 每次擊殺敵蛇獲得的積分           |
| `KILL_MASTERY_BONUS` | `SMALLINT UNSIGNED` |    2    |  點  | 每次擊殺敵蛇獲得的額外結算熟練度 |
| `RATING_BONUS_SSS`   | `SMALLINT UNSIGNED` |    50    |  點  | 評級 SSS 額外加成熟練度          |
| `RATING_BONUS_SS`    | `SMALLINT UNSIGNED` |    30    |  點  | 評級 SS 額外加成熟練度           |
| `RATING_BONUS_S`     | `SMALLINT UNSIGNED` |    10    |  點  | 評級 S 額外加成熟練度            |

#### 16.4 地圖與場景參數

| 參數名稱                      | 資料型態 | 預設數值 |  單位  | 參數描述                      |
| :---------------------------- | :------- | :------: | :----: | :---------------------------- |
| `MAP_WIDTH`                 | `INT UNSIGNED` |   2200   |   px   | 地圖邊界寬度                  |
| `MAP_HEIGHT`                | `INT UNSIGNED` |   2200   |   px   | 地圖邊界高度                  |
| `ROCK_BOUNCE_PENALTY_RATIO` | `INT UNSIGNED` |   5000   | 萬分比 | 撞邊界與岩石的長度扣除百分比  |
| `RIVER_SLOW_MULTIPLIER`     | `SMALLINT` |   -50   | 百分比 | 河流減速比例                  |
| `FOOD_DIST_ROCK`            | `INT UNSIGNED` |   5000   | 萬分比 | 食物岩石區分配權重            |
| `FOOD_DIST_RIVER`           | `INT UNSIGNED` |   3000   | 萬分比 | 食物河流區分配權重            |
| `FOOD_DIST_PLAINS`          | `INT UNSIGNED` |   2000   | 萬分比 | 食物平地區分配權重            |
| `FOOD_VAL_SMALL`            | `SMALLINT UNSIGNED` |    1    |   分   | 小食物積分價值                |
| `FOOD_VAL_MEDIUM`           | `SMALLINT UNSIGNED` |    3    |   分   | 中食物積分價值                |
| `FOOD_VAL_LARGE`            | `SMALLINT UNSIGNED` |    5    |   分   | 大食物積分價值                |
| `FOOD_VAL_DEBRIS`           | `SMALLINT UNSIGNED` |    10    |   分   | 死亡殘骸結晶積分價值          |
| `INITIAL_FOOD_POINTS`       | `INT UNSIGNED` |   2000   |   分   | 開局地圖隨機食物總分          |
| `MIN_FOOD_POINTS`           | `INT UNSIGNED` |   1000   |   分   | 食物補足下限                  |
| `FOOD_REPLENISH_INTERVAL`   | `INT UNSIGNED` |  10000  |   ms   | 食物補充檢測週期              |
| `DEBRIS_PERCENTAGE`         | `INT UNSIGNED` |   5000   | 萬分比 | 死亡/殘骸轉化比例             |
| `DEBRIS_LIFETIME`           | `INT UNSIGNED` |    10    |   s   | 死亡殘骸轉化食物生存/消失時間 |

#### 16.5 電腦 AI 策略參數

* **五大策略 AI 強度控制配置矩陣**：

| 電腦策略類型 `<br>VARCHAR(32)` | 反應延遲`<br>AI_REACTION_DELAY` `<br>INT UNSIGNED` | 轉彎精準度`<br>AI_STEERING_PRECISION` `<br>INT UNSIGNED` | 衝刺決策機率`<br>AI_DASH_PROB` `<br>INT UNSIGNED` | 避險檢測半徑`<br>AI_EVADE_DISTANCE` `<br>INT UNSIGNED` | 追逐/尋路半徑`<br>AI_CHASE_DISTANCE` `<br>INT UNSIGNED` |
| :---------------------- | :-------------------------------: | :-------------------------------------: | :------------------------------: | :-----------------------------------: | :------------------------------------: |
| **【初階 BOT】**  |            `300 ms`            |             `5000 (50%)`             |          `1000 (10%)`          |               `50 px`               |               `100 px`               |
| **【避險型 AI】** |            `150 ms`            |             `7000 (70%)`             |          `2000 (20%)`          |              `250 px`              |               `150 px`               |
| **【收集型 AI】** |            `120 ms`            |             `8000 (80%)`             |          `3000 (30%)`          |              `150 px`              |               `400 px`               |
...
| 參數名稱                       | 資料型態 | 預設數值 | 單位 | 參數描述                                 |
| :----------------------------- | :------- | :------: | :--: | :--------------------------------------- |
| `AI_EVADE_PANIC_DISTANCE`    | `INT UNSIGNED` |    80    |  px  | 電腦玩家危急逃生避險檢測半徑             |
| `AI_INTERCEPT_PREDICT_TIME`  | `DECIMAL(4,2) UNSIGNED` |   0.5   |  s  | 電腦玩家戰術卡位攔截預判時間             |
| `AI_DASH_RESOURCE_DISTANCE`  | `INT UNSIGNED` |   300   |  px  | 電腦玩家常態搶道具與高價食物半徑         |
| `AI_CHASE_ABORT_DISTANCE`    | `INT UNSIGNED` |   250   |  px  | 電腦玩家追擊/攔截中止之最大距離門檻      |
| `AI_CHASE_ABORT_TIME`        | `DECIMAL(4,2) UNSIGNED` |   1.0   |  s  | 電腦玩家追擊/攔截中止之最大時長門檻      |
| `AI_PROTECT_MIN_LENGTH`      | `INT UNSIGNED` |    80    |  節  | 電腦玩家低長度保護禁用衝刺之長度下限     |
| `AI_PROTECT_RECOVERY_LENGTH` | `INT UNSIGNED` |   120   |  節  | 電腦玩家解鎖限制、回復常態衝刺之長度上限 |

#### 16.6 局內道具直購參數

| 參數名稱                        | 資料型態 | 預設數值 |  單位  | 參數描述                                                      |
| :------------------------------ | :------- | :------: | :----: | :------------------------------------------------------------ |
| `ITEM_MAGNET_RADIUS`          | `INT UNSIGNED` |   100   |   px   | 地圖磁鐵吸附半徑                                              |
| `ITEM_MAGNET_DURATION`        | `INT UNSIGNED` |    10    |   s   | 地圖磁鐵持續時間                                              |
| `ITEM_MUSHROOM_DURATION`      | `INT UNSIGNED` |    10    |   s   | 地圖巨大蘑菇持續時間                                          |
| `ITEM_MUSHROOM_SIZE`          | `INT UNSIGNED` |  15000  | 萬分比 | 巨大蘑菇體型放大倍率                                          |
| `ITEM_MUSHROOM_SPEED_PENALTY` | `SMALLINT` |   -25   | 百分比 | 巨大蘑菇速度扣減                                              |
| `ITEM_LUCKY7_DURATION`        | `INT UNSIGNED` |    10    |   s   | 地圖幸運糖持續時間                                            |
| `ITEM_CANDY_MULTIPLIER`       | `INT UNSIGNED` |  30000  | 萬分比 | 幸運糖加成倍率                                                |
| `ITEM_EYE_DURATION`           | `INT UNSIGNED` |    10    |   s   | 地圖鷹眼持續時間                                              |
| `ITEM_EYE_SCALE`              | `INT UNSIGNED` |  13500  | 萬分比 | 鷹眼相機視野拉遠倍率                                          |
| `ITEM_SPAWN_INTERVAL`         | `INT UNSIGNED` |    20    |   s   | 道具刷新時間間隔                                              |
| `ITEM_MAX_COUNT`              | `SMALLINT UNSIGNED` |    12    |   個   | 同時存在最大道具上限                                          |
| `ITEM_TEASER_TIME`            | `INT UNSIGNED` |    2    |   s   | 道具刷新閃爍預告時長                                          |
| `ITEM_EYE_BUY_COST`           | `INT UNSIGNED` |   2000   |  金幣  | 局內直購鷹眼消耗金幣                                          |
| `ITEM_MAGNET_BUY_COST`        | `INT UNSIGNED` |   4000   |  金幣  | 局內直購磁鐵消耗金幣                                          |
| `ITEM_MUSHROOM_BUY_COST`      | `INT UNSIGNED` |   6000   |  金幣  | 局內直購巨大蘑菇消耗金幣                                      |
| `ITEM_CANDY_BUY_COST`         | `INT UNSIGNED` |   8000   |  金幣  | 局內直購幸運糖消耗金幣                                        |
| `BOOSTER_PRICE_MULTIPLIER`    | `INT UNSIGNED` |  15000  | 萬分比 | 局內直購同款道具每買一次的價格遞增比例 (1.5倍)                |
| `BOOSTER_LIMIT_INITIAL`       | `TINYINT UNSIGNED` |    2    |   次   | 局內直購同款道具單局初始次數上限 (成長之路 Lv 0 狀態)         |
| `BOOSTER_LIMIT_MAX`           | `TINYINT UNSIGNED` |    5    |   次   | 局內直購同款道具單局最大次數上限 (成長之路 Lv 53-56 分別解鎖) |
...
| `PASSIVE_SPEED_UPGRADE`         | `SMALLINT` |      -25      |       15       |      +1      | 百分比 | 巨大蘑菇跑速強化增幅     |
...
| `PASSIVE_RIVER_RESIST`          | `SMALLINT` |      -50      |       -10       |      +1      | 百分比 | 河流減速效果強化減免     |
| `PASSIVE_UPGRADE_START_COST`    | `INT UNSIGNED` |      1000      |        -        |      -      |  金幣  | 強化初始金幣費用 (x=0)   |
| `PASSIVE_UPGRADE_END_COST`      | `INT UNSIGNED` |     300000     |        -        |      -      |  金幣  | 強化滿級金幣費用 (x=399) |
| `PASSIVE_UPGRADE_SHAPE_K`       | `DECIMAL(6,3) UNSIGNED` |     0.024     |        -        |      -      |  比例  | S 曲線漲幅係數 k         |
| `PASSIVE_UPGRADE_INFLECTION_X0` | `SMALLINT UNSIGNED` |      190      |        -        |      -      |   次   | S 曲線拐點級數 x0        |

#### 16.8 個性化外觀與貼圖參數

| 參數名稱           | 資料型態 | 預設數值 | 單位 | 參數描述                   |
| :----------------- | :------- | :------: | :--: | :------------------------- |
| `EMOTE_DURATION` | `INT UNSIGNED` |   2500   |  ms  | 表情貼圖在蛇頭上方持續時間 |

---

### 17. 字串表與多語系文字參數

本章收斂遊戲內需要由程式、UI 或營運配置的文字字串。實作時建議以 String Table 管理，Key 使用英文固定命名，文字內容提供繁中與英文兩版。
