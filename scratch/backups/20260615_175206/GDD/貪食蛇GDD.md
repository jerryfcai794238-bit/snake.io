...
* **UI Flow 與示意圖同步**：
  * Ch2.1 補入配對中、教學模式、教學關卡、好友房完整回流流程。
  * 新增 `Ref_配對中.png` 作為配對中介面參考圖。
  * Ch3.3 新增 `Ref_第一名方位指示.jpg` 作為第一名方位指示專用圖。
...
* **美術需求同步**：
  * Ch18 重整為 UI 畫面、Icon、局中特效、Buff / Popup / Toast 需求。
  * 明確不新增貨幣 Icon 需求。
...
![第一名方位指示專用參考圖](<Reference Image/Ref_第一名方位指示.jpg>)
...
| 資源名稱 | 類型 | 參考圖 / 尺寸 | 說明 | 是否需要客製化 |
| :--- | :--- | :--- | :--- | :---: |
| **AppIcon** | Icon | 1024*1024 px | 2D 卡通蛇頭像，對齊玩星派對平台視覺。 | 需要 |
| **Loading 畫面** | 背景圖 | 2340*1080 px | 手機橫版 2D 貪食蛇競技插圖。 | 需要 |
| **大廳 UI 套件** | UI 向量 | `Ref_大廳.png` | 模式選擇卡片、能量顯示、熟練度、技能狀態、活動入口、被動強化、外觀商店、更換外觀、創建好友房、加入好友房、教學模式入口。 | 需要 |
| **配對中 UI** | UI 向量 | `Ref_配對中.png` | 顯示目前模式、等待狀態、隊伍人數、取消配對按鈕；好友房配對時需保留隊伍資訊。 | 需要 |
| **局中戰場 HUD 套件** | HUD 向量 | `Ref_對局中.png` | 搖桿、衝刺鍵、暴食鍵、高級道具快捷欄、排行榜、Buff 顯示列、第一名追蹤指針。 | 需要 |
| **對局結算 UI** | UI 向量 | `Ref_對局結算.png` | 排名、評價、擊殺、死亡、本局可獲得金幣。 | 需要 |
| **平台結算 UI** | UI 向量 | `Ref_平台結算.png` | 本局名次、實際入帳獎勵、平台 XP、外觀熟練度、活動入口、中途離開狀態。 | 需要 |
| **被動屬性 UI 套件** | UI 向量 | `Ref_被動強化介面.png` | 被動強化底框、10 個屬性圖示、隨機強化按鈕、金幣不足狀態、全滿級狀態、屬性詳情 Popup。 | 需要 |
| **成長之路 UI** | UI 向量 | `Ref_成長之路.png` | 成長節點、目前等級、下一級預覽、未達成、可領取/自動生效、已生效、升級演出。 | 需要 |
| **更換外觀 UI** | UI 向量 | `Ref_更換外觀-造型.png`、`Ref_更換外觀-貼圖.png` | 造型與表情貼圖分頁、已裝備、已擁有、未擁有、選取但未套用狀態。 | 需要 |
| **外觀商店 UI** | UI 向量 | `Ref_外觀商店-造型.png`、`Ref_外觀商店-貼圖.png` | 金幣商品、鑽石商品、已購買、未持有、貨幣不足、限時商品狀態。 | 需要 |
| **教學關卡選擇介面** | UI 向量 | `Ref_教學模式.png` | 縱向關卡列表滾動底框、關卡卡片、過關蓋章標誌、首次通關獎勵提示。 | 需要 |
| **隊伍大廳 UI 介面** | UI 向量 | `Ref_好友房.png`、`Ref_加入好友房.png` | 隊友列表、隊長標記、Ready 狀態、開始按鈕灰階、房間碼輸入、錯誤房碼提示。 | 需要 |

#### 18.2 Icon 需求

| 資源名稱 | 類型 | 參考圖 / 尺寸 | 用途 | 是否需要客製化 |
| :--- | :--- | :--- | :--- | :---: |
| **主動技：衝刺** | Icon | `Ref_Icon_主動技-衝刺.png` | 主動技能表、局中技能按鈕、成長之路解鎖提示。 | 已有 |
| **主動技：暴食** | Icon | `Ref_Icon_主動技-暴食.png` | 主動技能表、局中技能按鈕、成長之路解鎖提示。 | 已有 |
| **地圖/高級道具：磁鐵** | Icon | `Ref_Icon_道具-磁鐵.png` | 場上道具、Buff 列、高級道具按鈕。 | 已有 |
| **地圖/高級道具：巨大蘑菇** | Icon | `Ref_Icon_道具-巨大蘑菇.png` | 場上道具、Buff 列、高級道具按鈕。 | 已有 |
| **地圖/高級道具：幸運糖** | Icon | `Ref_Icon_道具-幸運糖.png` | 場上道具、Buff 列、高級道具按鈕。 | 已有 |
| **地圖/高級道具：鷹眼** | Icon | `Ref_Icon_道具-鷹眼.png` | 場上道具、Buff 列、高級道具按鈕。 | 已有 |
| **皇冠 / 第一名指示** | UI/掛飾 | `Ref_第一名方位指示.jpg`、32*32 px | 排行榜第一名、蛇頭皇冠、畫面邊緣追蹤指示。 | 需要 |
| **被動屬性 10 種圖示** | Icon | 32*32 px | 被動強化列表與屬性詳情 Popup。 | 需要 |
| **表情貼圖 5 種預設** | 貼圖 | 128*128 px | 擊殺、截斷、死亡、被截斷、擦身 5 種觸發情境。 | 需要 |
| **模式圖示** | Icon | 64*64 px | Solo、大亂鬥、陣營對抗、教學模式卡片。 | 需要 |

#### 18.3 局中特效與狀態演出

| 資源名稱 | 類型 | 對應章節 | 說明 | 是否需要客製化 |
| :--- | :--- | :--- | :--- | :---: |
| **衝刺加速特效** | 特效 | Ch4.5 | 速度線、蛇身拉伸、體力消耗視覺。 | 需要 |
| **減速河道特效** | 特效 | Ch4.5 / Ch6.2 | 河流區域、減速狀態、抗性減免提示。 | 需要 |
| **暴食力場** | 特效 | Ch4.3 | 吸附範圍圈、食物飛行軌跡、冷卻遮罩。 | 需要 |
| **地圖道具生成預告** | 特效 | Ch7.5 | 道具出現前 1500ms 倒數動畫預告。 | 需要 |
| **磁鐵吸附效果** | 特效 | Ch7.5 | 食物被吸向蛇頭的飛行與拖尾。 | 需要 |
| **巨大蘑菇體型變化** | 特效 | Ch7.5 | 蛇身膨脹、霸體光效、結束縮回。 | 需要 |
| **幸運糖加成效果** | 特效 | Ch7.5 | 蛇頭發光、得分加成跳字。 | 需要 |
| **鷹眼視野拉遠** | 特效 | Ch7.5 | 相機平滑拉遠與回復。 | 需要 |
| **死亡碎裂與殘骸** | 特效 | Ch5.4 | 蛇身碎裂、殘骸結晶生成。 | 需要 |
| **重生保護** | 特效 | Ch5.2 / Ch5.3 | 復活倒數、短暫保護罩或透明狀態。 | 需要 |
| **擊殺橫幅** | UI 特效 | Ch3.1.1.1 | 陣營色、玩家名、擊殺者/被擊殺者格式。 | 需要 |
| **表情貼圖演出** | 動畫 | Ch12.2 | 得意、大哭、生氣、求饒等貼圖的彈出、抖動、漸隱。 | 需要 |

#### 18.4 Buff / Popup / Toast 需求

| 資源名稱 | 類型 | 對應章節 | 說明 | 是否需要客製化 |
| :--- | :--- | :--- | :--- | :---: |
| **Buff 顯示列** | HUD UI | Ch8.2 | 上方 Buff icon、剩餘時間、環形倒數、滿級光框。 | 需要 |
| **Buff 新增演出** | UI 動畫 | Ch8.2 | icon 滑入、亮起或脈衝。 | 需要 |
| **Buff 覆蓋演出** | UI 動畫 | Ch8.2 / Ch8.3 | 閃光、縮放、刷新掃光、時間重置提示。 | 需要 |
| **高級道具按鈕四狀態** | HUD UI | Ch8.1.3 | 可使用、效果生效中、次數耗盡、金幣不足。 | 需要 |
| **中央漂浮提示** | Toast | Ch8.1.3 / Ch14.2 | 金幣不足、效果生效中、已達上限、房間碼錯誤、已離開隊伍。 | 需要 |
| **局中退出二次確認** | Popup | Ch2.1 / Ch3.4 / Ch15 | 文字 `僅獲得最低評價50%獎勵`，按鈕 `是 / 否`。 | 需要 |
| **被動強化規則說明** | Popup | Ch10.1.1 | `[i]` 說明彈窗，可捲動文字。 | 需要 |
| **被動屬性詳情** | Popup | Ch10.1.1 | 當前等級、下一級、效果說明。 | 需要 |
| **外觀 / 商店貨幣不足提示** | Toast | Ch12 / Ch13 | 未擁有不可套用、貨幣不足、購買成功提示。 | 需要 |
