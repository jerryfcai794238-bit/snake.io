...
* ＞ 10 節 (身體區) ➔ 受擊方被截斷。
* **擊殺廣播提示**：當觸發任何擊殺事件時，畫面中央上方會彈出置頂廣播提示（Tip），格式為 `"{擊殺者名稱} 擊殺 {被擊殺者名稱}"`。
...
* **遞增溢價邏輯**：每次使用後，下次使用所需的金幣消耗量固定增加「基礎價格的 50%」（溢價比例由變數 `BOOSTER_PRICE_MULTIPLIER = 15000` 萬分比控制）。
* **遞增消耗計算公式**：
...
* **優先級與覆蓋規則**：
  * **高級覆蓋普通**：
    * 當前生效效果為「地圖道具」（非滿級數值）時，玩家若點擊「高級道具」，後者會立刻**完全覆蓋**前者。
    * 「高級道具」的效果參數直接套用 **Lv 40 滿級參數**，且剩餘持續時間**重置為 10 秒滿額時間(受養成影響)**。
  * **滿級狀態點擊攔截**：
    * 當前生效效果為「高級道具」（已為滿級數值），或是玩家已達被動滿級（Lv 40）而處於滿級的「地圖道具」效果下，該道具的「高級道具}」按鈕會置為「狀態 B. 效果生效中」（見 Ch 6.1.3 狀態表，半透明置灰 50%）。
    * 系統將阻擋並攔截玩家的點擊動作，不消耗金幣，避免重複消耗與時效浪費。

* **重疊拾取與時效重置規則**：
  * **普通重複拾取**：
    * 當前為「地圖道具」效果（非滿級）生效中，若玩家再次拾取同種「地圖道具」，效果數值不變，剩餘持續時間**直接重置（刷新）為 10 秒最大值**。
  * **滿級狀態重複拾取**：
    * 當前為「高級道具」生效中，或已達被動滿級（Lv 40）的「地圖道具」生效中，若玩家再次拾取同種「地圖道具」，效果數值保持滿級不變，剩餘持續時間**直接重置（刷新）為 10 秒最大值**。
  * **地圖生效中點擊高級按鈕**：
    * 當前為已達被動滿級（Lv 40）的「地圖道具」效果生效中，由於此時效果與高級道具同級，玩家欲點擊該「高級道具」按鈕時，按鈕會**比照「狀態 B. 效果生效中」置灰阻擋**（見 Ch 6.1.3 狀態表），不允許點擊使用，避免重複消耗金幣。
