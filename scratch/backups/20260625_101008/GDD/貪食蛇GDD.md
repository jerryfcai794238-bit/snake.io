...
* **死亡局內掉落池**：蛇隻死亡時，系統會於蛇頭位置必定掉落可拾取地圖道具，包含金幣、星光卡包與活動道具等。掉落池需由 DataTable / 設定檔控制，且可依營運需求增減池內項目。
* **權重必掉規則**：每次死亡均觸發一次掉落判定，並依掉落池權重選出實際掉落內容；權重、各項物品的出現機率與掉落結果皆需可調整，避免改版時硬編碼。
* **金幣名次掉落**：死亡掉落中的金幣數量需依死亡蛇隻當前名次決定，名次對應的掉落量需可配置，方便區分高名次與低名次的局內死亡補償。
* **活動道具條件掉落**：活動道具不會永遠存在於掉落池中，僅在對應活動開啟時才允許掉落；活動結束後該道具從局內死亡掉落池中暫時移除。
* **平台掉落區分**：上述掉落屬於局內死亡掉落，與平台結算階段的隨機掉落獎勵不同；平台掉落仍維持對局結束後才發放，不與死亡瞬間掉落共用流程。
