...
* **殘骸價值與無條件捨去**：轉化比例由 `DEBRIS_PERCENTAGE = 50%`（由 [16.4 地圖與場景參數](#162-module) 控制）決定。當蛇隻死亡時，其個人累計長度積分的 50% 將轉化為地圖食物殘骸，並**無條件捨去至 10（單顆殘骸結晶積分）的倍數**。
...
* **地圖尺寸**：寬度 `MAP_WIDTH = 2200px`，高度 `MAP_HEIGHT = 2200px` （由 [16.4 地圖與場景參數](#162-module) 控制）。
* **岩石區**：地圖中隨機分布無法穿透 of 2D 岩石（碰撞規則詳見 5.1），撞擊不致死，觸發回彈並扣減 50% 當前長度。

#### 6.2 緩速河道設計

* **河流區**：河流緩速區，進入河流時移動速度減幅比例為 `RIVER_SLOW_MULTIPLIER = -5000` (即減速 50%，由 [16.4 地圖與場景參數](#162-module) 控制)。
...
* **地形投放分配權重**：（由 [16.4 地圖與場景參數](#162-module) 數據驅動）
...
* **五大策略 AI 強度控制配置矩陣**：（詳參 [16.5 電腦 AI 策略參數](#163-aistrategy)）
...
定義所有電腦玩家在執行常態尋路或戰術行為時的衝刺與防盲追邏輯，其決策預設值隨 AI 類型從強度配置矩陣中載入，細分為以下四層與中止機制（各參數常數詳參 [16.5 電腦 AI 策略參數](#163-aistrategy)）：
