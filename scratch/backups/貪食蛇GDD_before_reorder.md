...
* **流程與參數修改**：
  * 修改 Ch 6 Mermaid 流程圖為局內直購、次數與遞增判定。
  * 升級全文版號標記為 ``。
...
* **生死與碰撞規則修正**：
  * 修正 Ch 3.1 碰撞判定中「雙方皆衝刺頭對頭碰撞」，由原本的「扣除長度 50%」改為 **「反彈但不扣除長度」**。
* **安全生成重疊規則優化**：
  * 修正 Ch 5.1 食物與隨機道具安全生成間距，由 `15px` 縮減為 `2px`。食物與食物、食物與道具、道具與道具可以重疊，但不能完全重疊。
* **戰前攜帶道具規則與流程圖**：
  * 於 Ch 6.1 明確列出可攜帶的四種道具，並限制初始最大攜帶上限為 1。
...
* **被動養成與成長之路重構**：
  * 刪除 Ch 9.1 原有的成長之路系統說明。
  * 新增 Ch 9.3 成長之路與主動技能升級系統，詳細說明玩遊戲累積熟練度提升成長之路等級，進而解鎖與升級主動技能的機制，並以 Mermaid 流程圖表現。
* **外觀商店拆分與更換外觀規格**：
  * 將外觀商店獨立拆出為全新大章節 `## 12. 外觀商店與皮膚定價規則`，順延後續章節。
  * 於 Ch 10.4 新增更換外觀與表情貼圖的操作入口與配置規格說明。
...
* **AI 策略與對局補位配置表格化**：
  * Ch 8.2 AI 策略表移除所有 `<br>` 標記，改用 inline 括號。
  * 將對局電腦固定配比重寫為標準 Markdown 表格，欄位為 `模式｜電腦數｜初階｜避險｜收集｜截斷｜追獵｜說明與限制`。
  * 修正 Solo 電腦數量為 7 隻，且對局補位配置與加總完全對齊（2 初階 + 2 避險 + 1 收集 + 1 截斷 + 1 追獵 = 7 隻）。
* **主動技能規則修正**：
  * 修正 Ch 2.2 衝刺技能，移除超載狀態中「拾取蘇打解鎖」描述。
  * 修正 Ch 2.2 暴食，移除隨成長之路解鎖次數設定，改為「解鎖並常駐升級（增加半徑、減少冷卻）」。
* **生死與碰撞規則修正**：
  * 修正 Ch 3.1 碰撞判定矩陣，雙方衝刺頭對頭碰撞改為 **「雙方皆判定死亡」**。
  * 於 Ch 3.1 碰撞判定中明確寫入碎裂殘骸僅轉化生前積分之 `DEBRIS_PERCENTAGE = 5000` (50% 萬分比) 的食物積分。
* **結構大改拆分**：
  * 將原 Ch 4 拆分為三個大章：Ch 4 地圖與環境系統、Ch 5 食物、道具與積分系統、Ch 6 戰前攜帶道具與堆疊規則。
  * 將原 5.4 中的長度與積分成長關係，單獨拉出為獨立小節 `### 6.4 長度成長關係`。
  * 後續章節編號順延至 Ch 14，並同步更新文內所有交互參照。
...
* **公式渲染修復**：
  * 將 Ch 2.1 跑速公式由 LaTeX 方程式塊改為 Plain Text 格式，修復部分 Markdown 解析器渲染異常。
* **中離懲罰放寬**：
  * 移除 Ch 9.2 中離 5 分鐘禁止匹配限制，玩家中離後仍可立即再次發起配對。
* **版號與標記更新**：
...
* **參數與顯示修正**：
  * 修正 Ch 7.2 磁吸半徑顯示為純數字，移除冗餘單位。
  * 修正 Ch 7.2 岩石抗性與河流抗性數值，將「扣減/減速」描述統一改為負號（`-4900` 與 `-1000`）。
  * 修正 Ch 8.2 表情持續時長 `EMOTE_DURATION` 從 3s 調整為 `2500ms`，同步更新 Ch 10 參數總覽。
...
* **結構大改與精簡**：
  ...
  * 將碰撞生死判定、死亡復活及幽靈重生獨立為 Chapter 3，緊跟在基礎操作之後。
...
* **規則與數值微調**：
  ...
  * 將原 8.4 結算評價與獎勵移入 Ch 5，刪除原 8.5 經濟章節並將能量門票消耗移入 Ch 5 模式表格。
...
| :------------------------ | :---------------------- | :------------------- | :----------------------------------------------------------------------- | :-------------------------------------------------- | :------------------------------------------------ |
| **衝刺 (Dash)**     | `<Module表_基礎屬性>` | 無須解鎖 (初始 Lv 0) | **體力上限**：可升至 Lv4`<br>`**體力回復速度**：可升至 Lv4 | 每秒消耗 `333` 點體力。加速 `+25%` 移動速度。   | 體力歸 0 進入超載：鎖定禁衝刺與回體，回滿後解鎖。 |
...
## 3. 碰撞、死亡與重生復活規則 (Collision, Death & Respawn)
...
|   頭   |     身     |    衝刺    |   未衝刺   |     X     |     X     |     -     |     -     | 受擊方**從撞擊點截斷**變為殘骸。 |
...
* **岩石區**：地圖中隨機分布無法穿透的 2D 岩石（碰撞規則詳見 3.1），撞擊不致死，觸發回彈並扣減 50% 當前長度。
...
* **自動吸附半徑**：常態基礎吸附半徑為 `BASE_SUCTION_RADIUS = 50px`（由 `<Module表_基礎屬性>` 控制）。當食物或殘骸進入此半徑時，自動啟動 Lerp 內插向蛇頭滑行並被吞食。
...
* **個人長度積分（物理成長線）**：
  ...
  * 通過吞食地圖基礎食物或死者殘骸資源累積。
  * 當前個人長度積分每達到「每節所需積分」`POINTS_PER_SECTION = 10` 分（由 `<Module表_基礎屬性>` 控制），蛇身長度增加 1 節。
  * 單次吞食得分加算公式：
    $$
...
* **擊殺積分（非成長競技線）**：
  ...
  * 玩家每成功擊殺一隻敵蛇，系統直接加算 `KILL_SCORE = 100` 分（由 `<Module表_遊戲模式>` 控制）。
  * 擊殺積分僅會直接加算至「對局總積分」中，**絕對不增加蛇身的物理長度與節數**。

### 5.4 長度成長關係

* **初始長度與重置基準**：初始長度 `INITIAL_LENGTH = 50` 節（由 `<Module表_基礎屬性>` 控制）。當玩家死亡並重生時，蛇頭與蛇身節數立刻重置回此初始狀態。
* **長度與積分換算公式**：
  $$
  \text{最終蛇隻總節數} = \text{初始長度} + \left\lfloor \frac{\text{當前個人長度積分} - \text{重生基準分}}{\text{每節所需積分}} \right\rfloor
* **重生基準分與平衡換算**：
  * 系統預設重生基準分為 `INITIAL_SCORE = 500` 分（由 `<Module表_基礎屬性>` 控制）。
...
* 地圖隨機安全座標每 `ITEM_SPAWN_INTERVAL = 20000ms`刷新一個地圖道具（由 `<Module表_遊戲模式>` 控制）。
* 地圖上道具上限為 `MAX_MAP_ITEMS = 12` 個（由 `<Module表_遊戲模式>` 控制）。
* 生成前有 `ITEM_PREVIEW_DURATION = 1500ms` 的倒數動畫預告（由 `<Module表_遊戲模式>` 控制）。
...
* **次數上限解鎖**：
  ...
  * 隨大廳【成長之路】等級提升逐步解鎖，單局最高可提升至 **5 次**（詳細解鎖對照表見 Ch 10.2）。
...
2. **滿級狀態點擊攔截（防呆）**：只要當前 Buff強度已是 Lv.40 滿級（不論是透過高級道具觸發，還是玩家被動已升滿 Lv.40 從地圖撿的），高級按鈕會進入 **「B. 效果生效中」半透明置灰 50% 狀態**，點擊時直接攔截，不扣費，防止玩家浪費金幣。
...
> 各對局模式電腦 BOT 補位固定分配與配比請參閱 Ch 8.2 對應表格。
...
* **同分破平 (Tie-breaker by Timestamp)**：若局終結算時有多名玩家的分數完全相同，名次將依據**達到該分數的毫秒級時間戳 (`score_update_timestamp`) 進行判定，先達到該分數 the 玩家排名在前**。系統會在每次得分時記錄當前幀的絕對時間戳（精確至毫秒），確保破平邏輯 100% 精準無爭議。 📝[v12.0]

### 7.3 局內排行榜與第一名冠冕顯示 📝[v12.0]
