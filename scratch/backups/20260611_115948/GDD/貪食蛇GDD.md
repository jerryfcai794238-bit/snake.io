...
* **重生基準分與平衡換算**：
  ...
  * 依據此公式平衡，玩家在重生基準之上，每吞食累積 10 分個人長度積分即可增長 1 節長度（等同於需吞食：小食物 10 顆 / 中食物 4 顆可達 12 分 / 大食物 2 顆）。
...
* **遞增溢價邏輯**：`BOOSTER_PRICE_MULTIPLIER` 中文名稱為「高級道具價格倍率」，數值 `15000` 代表每次使用後的最終價格倍率為 1.5 倍。若只看「增加幅度」，則為 `5000` 萬分比，也就是每次比基礎價格多加 `50%`。
* **遞增消耗計算公式**：

  ```text
  第 N 次使用需要消耗的金幣 = floor(基礎價格 + 基礎價格 * 高級道具加價幅度 * (N - 1))

  高級道具加價幅度 = (高級道具價格倍率 - 10000) / 10000
  高級道具價格倍率 = BOOSTER_PRICE_MULTIPLIER = 15000
  因此高級道具加價幅度 = 5000 / 10000 = 50%
  其中 N >= 1
  ```
* **各道具單局使用次數與金幣消耗對照表**：
