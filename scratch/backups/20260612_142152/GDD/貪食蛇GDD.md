...
* **對局結算金幣發放**：
  * 評價金幣獎勵改由遊戲/對局結算畫面計算、顯示並發放，因其與發育評級直接掛鉤。
  * 對局結算畫面需顯示玩家本局獲得金幣。
* **結算圖片更新**：
...
---
* **離開與結算**：局中點擊退出遊戲時，彈出二次確認 Popup，文案為「無法獲得評價和獎勵」，按鈕為「是 / 否」。選「否」返回局中；選「是」視為中途離開，跳過對局結算與評價金幣發放，直接進入平台結算流程。單局時間截止後則進入「遊戲結算彈窗」，展示各玩家(電腦)排名、自己發育評價、擊殺數、死亡數與本局獲得金幣，點擊後進入「平台結算介面」，展示平台等級、熟練度、隨機掉落獎勵、活動入口（如貼紙派對），並提供「成長之路」與「再來一局」之引導。


![對局結算介面參考圖](<Reference Image/Ref_對局結算.png>)

![平台結算介面參考圖](<Reference Image/Ref_平台結算.png>)
---
...
對局結束後，系統彈出對局結算畫面（介面佈局見下方參考圖），條列式展示以下局內遊戲內容。評價金幣獎勵與發育評級直接掛鉤，需於此畫面完成計算、顯示並發放；熟練度、平台 XP、隨機掉落與活動入口則由後續平台結算流程處理。
...
* **本局獲得金幣**：展示依發育評級計算出的評價金幣獎勵，並於對局結算畫面發放至玩家帳號。

##### 3.4.1 金幣獎勵機制

根據玩家的單局結算評級發放固定金幣獎勵，與排名無直接關聯。金幣獎勵於「遊戲/對局結算」階段計算、顯示並發放；平台結算不再作為評價金幣的發放點。
...
當玩家關閉「對局結算」視窗後，系統會自動載入「平台結算介面」（介面佈局與按鈕引導見下方參考圖）進行平台 XP、外觀熟練度、隨機掉落、活動入口與後續引導等局外流程。平台結算不發放與發育評級掛鉤的評價金幣。
