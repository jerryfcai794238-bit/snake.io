...
**文件版本**：18.0
**更新日期**：2026-06-25
**本次更新**：新增蛇隻死亡後的局內掉落池規則，將局內掉落與平台結算掉落分流，並同步更新版號。
...
* **追逐／尋路半徑與追擊中止規則**：
  ...
  * 追擊中不再使用固定距離常數。若目標距離超過該 AI 類型的 `AI_CHASE_RADIUS * 1.2`，視為脫離有效追逐範圍，AI 必須放棄當前戰術並回到常態尋路。
  * `AI_CHASE_ABORT_TIME` 是**追擊最長持續時間**：AI 已進入追擊、衝刺截斷或攔截演算後，若超過 `AI_CHASE_ABORT_TIME`（預設 `3.0 s`）仍未成功接近、截斷或形成有效攻擊角度，視為追擊失敗。
...
3. **攔截中止與防盲追機制 (關鍵優化)**：
   * 在進行衝刺截斷或追擊時，每影格進行即時演算。若滿足以下任一條件，判定攔截失敗，**AI 電腦玩家必須立刻終止衝刺加速、降回常速，並切換回常態尋路（停止無效盲目追擊）**：
     * **距離拉開**：敵蛇透過衝刺或大轉向，使雙方蛇頭距離拉開至 `> AI_CHASE_RADIUS * 1.2`。
     * **時效超限**：追擊或截斷持續時間超過 `AI_CHASE_ABORT_TIME`（預設 `3.0 s`），仍未成功接近、截斷或形成有效攻擊角度。
...
* **動態切換決策矩陣**：
  ...
  | **道具爭奪** | 當前蛇頭距離 `300 px` 內刷新/存在地圖道具                 | 暫停當前尋路，轉向直奔道具點，爭奪資源。              |
  | **保命迴避** | 當前蛇頭距離 `150 px` 內有敵蛇接近或執行衝刺              | 立即執行大轉向朝遠離威脅點方向滑行逃跑。              |
...
* **AI 決策優先級**：
  ...
  | 4 | 戰術追擊 / 截斷 | 目標位於該 AI 類型的 `AI_CHASE_RADIUS` 內，且符合追獵或截斷型 AI 的長度、角度與目標條件。 | 嘗試預判目標前進方向並切入；追擊中若距離超過 `AI_CHASE_RADIUS * 1.2` 或時間超過 `AI_CHASE_ABORT_TIME`，必須放棄。 |
...
> [!NOTE]
> 例如：即便初始分配的是高階的「追獵型 AI」，一旦其偵測到蛇頭距離 150px 內有威脅，仍會優先切換為「保命迴避」以確保生存，絕不愚蠢撞擊。
...
|  | 46 | ai | AI 追擊中止時間 | AI_CHASE_ABORT_TIME | 3000 | ms |
|  | 47 | ai | AI 低長度禁用衝刺門檻 | AI_PROTECT_MIN_LENGTH | 80 | 節 |
|  | 48 | ai | AI 恢復衝刺門檻 | AI_PROTECT_RECOVERY_LENGTH | 120 | 節 |
|  | 49 | item | 地圖道具刷新間隔 | ITEM_SPAWN_INTERVAL | 20 | s |
|  | 50 | item | 地圖道具同時存在上限 | ITEM_MAX_COUNT | 12 | 個 |
|  | 51 | item | 道具刷新預告時間 | ITEM_TEASER_TIME | 2 | s |
|  | 52 | item | 同款道具價格倍率 | BOOSTER_PRICE_MULTIPLIER | 15000 | 萬分比 |
|  | 53 | item | 高級道具初始單局上限 | BOOSTER_LIMIT_INITIAL | 2 | 次 |
|  | 54 | item | 高級道具最大單局上限 | BOOSTER_LIMIT_MAX | 5 | 次 |
|  | 55 | passive | 被動強化初始費用 | PASSIVE_UPGRADE_START_COST | 1000 | 金幣 |
|  | 56 | passive | 被動強化最高費用 | PASSIVE_UPGRADE_END_COST | 300000 | 金幣 |
|  | 57 | passive | S 曲線係數，24 代表 0.024 | PASSIVE_UPGRADE_SHAPE_K | 24 | 千分比 |
|  | 58 | passive | S 曲線拐點 | PASSIVE_UPGRADE_INFLECTION_X0 | 190 | 次 |
|  | 59 | cosmetic | 表情貼圖顯示時間 | EMOTE_DURATION | 2500 | ms |
|  | 60 | core | 蛇頭碰撞半徑 | SNAKE_HEAD_COLLISION_RADIUS | 12 | px |
|  | 61 | core | 蛇身碰撞半徑 | SNAKE_BODY_COLLISION_RADIUS | 8 | px |
|  | 62 | core | 蛇節視覺間距 | SNAKE_SECTION_SPACING | 10 | px |
|  | 63 | combat | 截斷判定避開蛇頭前段節數 | CUT_HEAD_SAFE_SECTIONS | 10 | 節 |
|  | 64 | map | 地圖物件生成最小安全偏移 | Object_SPAWN_MIN_DIST | 2 | px |
|  | 65 | item | 巨大蘑菇體型放大倍率 | ITEM_MUSHROOM_SIZE | 15000 | 萬分比 |
|  | 66 | ui | 局內排行榜顯示行數 | RANKING_DISPLAY_COUNT | 4 | 行 |
|  | 67 | ui | 對局倒數警示門檻 | TIME_WARNING_THRESHOLD | 30 | s |
|  | 68 | mode | 中離最低評價獎勵比例 | EARLY_LEAVE_REWARD_RATIO | 5000 | 萬分比 |
|  | 69 | map | 殘骸分數捨去倍數 | DEBRIS_SCORE_ROUND_UNIT | 10 | 分 |
|  | 70 | social | 房間碼位數 | ROOM_CODE_DIGITS | 5 | 位 |
|  | 71 | social | 好友房最少開始人數 | ROOM_MIN_PLAYERS | 2 | 人 |
|  | 72 | social | 好友房隊伍人數上限 | ROOM_MAX_PLAYERS | 4 | 人 |
...
*Powered by Antigravity v18.0 - 玩星派對專用*
