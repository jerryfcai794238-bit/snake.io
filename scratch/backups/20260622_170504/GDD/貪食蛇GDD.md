...
* **好友房互動規則補充**：
  * Ch14.3 補充隊友準備後的視角差異：當事人自己的按鈕切換為「取消準備」，隊長與其他非當事人仍看到該隊友狀態為「已準備」。
  * 字串表已保留 `準備`、`取消準備`、`開始配對` 等好友房相關文字參數，便於 UI 狀態對應。
* **中離與退出確認示意圖同步**：
  * Ch3.4 補入 `Ref_中離二次彈窗.png`，作為局中退出二次確認 Popup 的示意圖。
  * 中離規則維持：玩家確認退出後不進入一般對局結算，改由平台結算依最低評價獎勵 50% 顯示並入帳。
* **HUD 與局中狀態補充**：
  * Ch1.2 與美術需求同步補充蛇頭環狀體力槽 HUD：正常與消耗中為黃色，超載狀態為紅色，僅玩家自己可見。
  * 美術需求補充局中貼圖顯示、Buff 倒數閃爍、第一名皇冠指示物等局中 UI 狀態。
* **美術需求文件同步更新**：
  * [貪食蛇美術需求.md](貪食蛇美術需求.md) 補入 `Ref_中離二次彈窗.png` 作為退出確認 Popup 參考圖。
  * Ch3.3 皇冠 / 第一名 Icon 參考圖更新為 `Ref_皇冠指示物.png`。
  * Ch3.3 被動技能 Icon 更新為磁鐵半徑、磁鐵時間、巨大蘑菇跑速、巨大蘑菇時間、幸運糖倍率、幸運糖時間、鷹眼範圍、鷹眼時間、岩石抗性、河流抗性等 10 張圖文對照。
  * Ch4 地圖戰場需求改為四張地圖示意圖對照：草原冒險、河岸水域、岩石峽谷、糖果樂園，並保留各主題美術說明與驗收重點。
  * Ch2.7 被動強化表格刪除空的「參考圖路徑」欄位，避免外包閱讀混淆。
* **版號升級**：文件版本升級為 `16.0`。

</details>

<details>
<summary><b>v15.0 (2026-06-15) - UI Flow、斷線中離、示意圖與美術需求同步版</b></summary>

* **UI Flow 與示意圖同步**：
  * Ch2.1 補入教學模式、教學關卡、好友房完整回流流程。
  * Ch2.1 明確區分 Solo 直接開始，大亂鬥與陣營對抗才進入配對中。
  * `Ref_配對中.png` 改由好友房與美術需求文件等對應章節說明。
  * Ch3.4.6 新增 `Ref_平台結算-失敗.png`，中途離開名次顯示為「再接再厲」。
  * Ch3.3 保留 `Ref_對局中.png` 作為排行榜與第一名方位說明圖，移除獨立第一名方位指示圖。
  * Ch3.1 電腦 BOT 補位引用修正為 Ch9.1.1。
* **斷線與中離規格調整**：
  * 明確不做玩家斷線重連；斷線、關閉頁面、主動離開皆直接視為中離。
  * 中離玩家的局內蛇隻由避險型、收集型、截斷型、追獵型 4 種 AI 隨機一種接手。
  * 中離後跳過一般對局結算並進入平台結算。
  * 中離只取得最低評價獎勵的 50%，以目前 E 評價金幣 `1600` 計算，中離可獲得 `800` 金幣。
  * 局中退出二次確認文案調整為「僅獲得最低評價50%獎勵」。
* **UI 狀態與流程補充**：
  * Ch1.2 補離開按鈕與局內排行榜左上角位置。
  * Ch2.2 精簡大廳說明，移除公版上方資訊列、被動強化與離開結算重複說明。
  * Ch5.1 補碰撞與生死判定流程圖。
  * Ch10 補被動強化金幣不足、全滿級、隨機命中演出與 Popup 需求。
  * Ch10 新增 `Ref_強化演出.mp4` 作為點擊強化演出參考。
  * Ch11 補成長之路節點狀態、下一級預覽與等級提升演出。
  * Ch12 / Ch13 補外觀、貼圖、商店商品狀態。
  * Ch14 補好友房配對中、取消配對、踢人、隊長移交、房間碼失效與對局後 Ready 重置。
  * Ch17 新增字串表與多語系文字參數。
* **美術需求同步**：
  * 原 Ch17 音效與 Ch18 美術需求拆出為 [貪食蛇美術需求.md](貪食蛇美術需求.md)。
  * 美術需求新增 UI 圖文說明、Icon 圖文對照、特效需求與音效需求。
  * 音效需求移除語音包、SSS 評價專用 SE，新增截斷音效與死亡受擊音，配對成功改為輕快提示音。
  * 明確不新增貨幣圖示需求。
...
* **局中退出二次確認**：
  * 局中點擊退出遊戲時，先顯示 Popup 二次確認視窗，文案為「僅獲得最低評價50%獎勵」，選項為「是 / 否」。
  * 選擇「否」返回局中；選擇「是」視為中途離開，跳過一般對局結算並進入平台結算流程。
  * 中途離開僅取得最低評價獎勵的 50%，由平台結算顯示並入帳。
* **結算金幣顯示與入帳**：
...
* **斷線與中離規則**：
  * 現行規格不做斷線重連；玩家網路斷線、關閉頁面或主動離開，均直接視為中離。
  * 中離蛇隻立即判定死亡並依死亡碎裂規格產生食物殘骸；中離玩家僅取得最低評價獎勵的 50%。
* **結算熟練度與金幣調整**：
...
|  D  | 100 - 299      |   1800   |       0       |
|  E  | 99 以下        |   1600   |       0       |
> 來源讀取：[<RatingReward表>](#166-ratingreward)。評級門檻、金幣獎勵與評級額外熟練度皆以此表為唯一真源。
...
graph TD
    ...
    F -->|選擇關卡並進入| G{關卡選擇}
    G -->|關卡 1| H[教學關卡 1 - 移動與成長]
    G -->|關卡 2| I[教學關卡 2 - 衝刺與擊殺]
    G -->|關卡 3| J[教學關卡 3 - 暴食]
    H --> N{通關?}
    I --> N
    J --> N
...
> 關卡設定來源讀取：[<TutorialLevel表>](#169-tutoriallevel)；圖卡步驟順序與文字 Key 來源讀取：[<TutorialStep表>](#1610-tutorialstep)。

| 關卡                                      | 場景                                                                | 目標                                                                                    | 引導內容                                                                                                                                                                                                                                                                                                                         | 完成條件                                                              | 失敗條件                                                   |
| :---------------------------------------- | :------------------------------------------------------------------ | :-------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------- | :--------------------------------------------------------- |
| **教學關卡 1：移動與成長** | 小型封閉沙盒（約 1200×1200 px），無敵人、無岩石、無河流，場上散落高辨識度食物。 | 教導基本方向操作、吃食物及蛇身成長的因果關係。 | 1. 顯示關卡 1 圖卡。 <br> 2. 左下搖桿區出現手指光暈，引導玩家拖曳並完成轉向。 <br> 3. 高亮前方食物並顯示前往虛線，引導玩家吞食。 <br> 4. 吞食後放慢蛇身增長演出，強調新增蛇節。 | 完成至少 1 次方向轉換、吃到 10 顆食物，並觸發至少 1 次蛇身增長。 | 無。撞牆僅彈回，不致死亦不扣分。 |
| **教學關卡 2：衝刺與擊殺** | 封閉戰鬥沙盒，配置 2 隻低攻擊性初階 BOT 與足量安全空間；玩家初始體力為滿值。 | 教導按下衝刺、體力消耗與超載、蛇隻加速，以及利用衝刺截斷／擊殺對手。 | 1. 顯示關卡 2 圖卡。 <br> 2. 高亮衝刺按鈕，引導玩家按住衝刺。 <br> 3. 按住期間聚焦蛇頭環狀體力槽，呈現體力持續減少與歸零超載。 <br> 4. 體力恢復後再次按住衝刺，以速度線與殘影強調蛇隻加速；BOT 進入預設截斷路徑，引導玩家截斷或擊殺對手。 | 完成 1 次衝刺、進入 1 次超載、辨識蛇隻加速狀態，並成功截斷或擊殺 BOT 1 次。 | 無。BOT 不主動擊殺玩家；玩家死亡時原地復活並重置當前戰鬥步驟。 |
| **教學關卡 3：暴食** | 小型封閉沙盒，玩家周圍配置一圈超出基礎吸附半徑、但位於暴食 Lv 0 半徑內的食物。 | 教導使用暴食、辨識瞬間大範圍吸收效果，以及理解技能 CD。 | 1. 顯示關卡 3 圖卡。 <br> 2. 暴食按鈕預設可用並高亮，引導玩家點擊。 <br> 3. 觸發 Lv 0 暴食力場，以明顯範圍圈與吸附軌跡呈現瞬間大範圍吸收。 <br> 4. 按鈕進入 30 秒 CD，顯示遮罩與倒數，引導玩家等待 CD 結束後再次可用。 | 使用暴食 1 次、吸附至少 10 顆食物，並完整觀察暴食進入 CD 狀態。 | 無。暴食於教學內直接可用，不檢查成長之路解鎖狀態。 |

##### 3.5.4 教學圖卡資訊與製作需求

> [!NOTE]
> 版型與資訊層級參考使用者提供的圖 1（`230e7ba6f23cbc37a9956634ab4ef035.jpg`）；正式素材需依《貪食蛇》HUD、角色、技能 Icon 與美術風格重製，不可沿用參考圖中的 Logo、角色或獎勵素材。
...
| **體力回復速度** |  `80`  |   點/s   | 停止衝刺後每秒自動恢復的體力量               | `STAMINA_REGEN_SPEED`         |
...
* **基礎跑速**：初始基礎跑速單位為 `BASE_SPEED = 400`（等同於 `4.00 px/s`，即以 100 代表 1 px/s，由 [16.2 基礎核心參數](#162-module) 控制）。
* **速度計算公式**：
...
| :-----------------------------------------------: | :------------------------ | :---------------------------------- | :------------------- | :------------------------------------------------------------------------ | :-------------------------------------------------- | :------------------------------------------------ |
| ![衝刺](<./Reference Image/Ref_Icon_主動技-衝刺.png>) | **衝刺 (Dash)**     | [16.2 基礎核心參數](#162-module) | 無須解鎖 (初始 Lv 0) | **體力上限**：可升至 Lv4  <br> **體力回復速度**：可升至 Lv4 | 每秒消耗 `333` 點體力。加速 `+25%` 移動速度。   | 體力歸 0 進入超載：鎖定禁衝刺與回體，回滿後解鎖。 |
| ![暴食](<./Reference Image/Ref_Icon_主動技-暴食.png>) | **暴食 (Gluttony)** | [16.2 基礎核心參數](#162-module) | 成長之路 Lv 1 解鎖   | **吸附範圍**：可升至 Lv4  <br> **冷卻時間縮短**：可升至 Lv4 | 瞬間釋放暴食力場，以速度 `12 px/s` 吸附區域食物。 | 釋放後進入冷卻。單局無次數限制。                  |
...
* **殘骸價值與無條件捨去**：轉化比例由 `DEBRIS_PERCENTAGE = 50%`（由 [Ch16.2 Module](#162-module) 控制）決定。當蛇隻死亡時，其個人累計長度積分的 50% 將轉化為地圖食物殘骸，並**無條件捨去至 10（單顆殘骸結晶積分）的倍數**。
...
* **地圖尺寸**：寬度 `MAP_WIDTH = 2200px`，高度 `MAP_HEIGHT = 2200px` （由 [Ch16.2 Module](#162-module) 控制）。
* **岩石區**：地圖中隨機分布無法穿透 of 2D 岩石（碰撞規則詳見 5.1），撞擊不致死，觸發回彈並扣減 50% 當前長度。

#### 6.2 緩速河道設計

* **河流區**：河流緩速區，進入河流時移動速度減幅比例為 `RIVER_SLOW_MULTIPLIER = -5000` (即減速 50%，由 [Ch16.2 Module](#162-module) 控制)。
...
* **安全投放與防穿幫重疊**：食物與道具生成時，必須確保其碰撞半徑完全不與岩石或地圖邊界接觸（即物理無碰撞邊緣點）。
  ...
  * 食物與食物、食物與地圖道具、地圖道具與地圖道具之間**允許重疊，但不可完全等同**（即兩者的中心點座標 $X, Y$ 不能完全重合，需由最小間距 `Object_SPAWN_MIN_DIST = 2px` 進行安全隨機偏移控制；來源讀取：[<Module表>](#162-module)）。
* **地形投放分配權重**：（由 [Ch16.2 Module](#162-module) 數據驅動）
...
* **自動吸附半徑**：常態基礎吸附半徑為 `BASE_SUCTION_RADIUS = 50px`（由 [16.2 基礎核心參數](#162-module) 控制）。當食物或殘骸進入此半徑時，自動啟動 Lerp 內插向蛇頭滑行並被吞食。
...
* **全域食物規格對照表**：
  ...
  > 食物分數、碰撞半徑與渲染尺寸來源讀取：[<Food表>](#167-food)。
...
* **個人長度積分（物理成長線）**：
  ...
  * 通過吞食地圖基礎食物或死者殘骸資源累積。
  * 當前個人長度積分每達到「每節所需積分」`POINTS_PER_SECTION = 10` 分（由 [16.2 基礎核心參數](#162-module) 控制），蛇身長度增加 1 節。
  * 單次吞食得分加算公式：
    $$
...
* **擊殺積分（非成長競技線）**：
  ...
  * 玩家每成功擊殺一隻敵蛇，系統直接加算 `KILL_SCORE = 100` 分（由 [16.3 遊戲模式參數](#162-module) 控制）。
  * 擊殺積分僅會直接加算至「對局總積分」中，**絕對不增加蛇身的物理長度與節數**。

#### 7.4 長度成長關係

* **初始長度與重生重置**：初始長度 `INITIAL_LENGTH = 50` 節（由 [16.2 基礎核心參數](#162-module) 控制）。當玩家死亡並重生時，蛇頭與蛇身節數立刻重置為 50 節。
...
* **投放頻率**：系統每 `ITEM_SPAWN_INTERVAL = 20000ms` 嘗試刷新 1 個地圖道具（由 [16.3 遊戲模式參數](#162-module) 控制）。
* **同時存在上限**：地圖上最多同時存在 `ITEM_MAX_COUNT = 12` 個地圖道具（由 [16.6 局內道具直購參數](#165-passiveskill) 控制）；若已達上限，該次刷新跳過。
* **投放座標邏輯**：地圖道具會投放在「隨機安全座標」上。隨機安全座標指系統從可行走地圖區域中隨機抽取的位置，該位置不得位於岩石、邊界外或其他不可通行區，且不得與既有食物或地圖道具完全同座標；若抽取失敗，系統重新抽取直到找到合法位置或達到嘗試上限。
* **重疊容許規則**：食物與地圖道具可以視覺上接近或部分重疊，但中心點不可完全相同，需套用 `Object_SPAWN_MIN_DIST = 2px` 的最小安全偏移（來源讀取：[<Module表>](#162-module)）。
* **投放預告**：生成前有 `ITEM_TEASER_TIME = 2s` 的閃爍預告（由 [16.6 局內道具直購參數](#165-passiveskill) 控制）。
...
> 出現機率來源讀取：[Ch16.3 AIStrategy](#163-aistrategy) 的 `SpawnWeight` 欄位。
...
> 來源讀取：[<ModeBotComposition表>](#168-modebotcomposition)。
...
* **五大策略 AI 強度控制配置矩陣**：（詳參 [Ch16.3 AIStrategy](#163-aistrategy)）
...
定義所有電腦玩家在執行常態尋路或戰術行為時的衝刺與防盲追邏輯，其決策預設值隨 AI 類型從強度配置矩陣中載入，細分為以下四層與中止機制（各參數常數詳參 [Ch16.3 AIStrategy](#163-aistrategy)）：
...
* **AI 決策優先級**：
  ...
  | :---: | :--- | :--- | :--- |
  | 1 | 保命迴避 | 蛇頭前方或半徑 `AI_EVADE_DISTANCE` 內存在岩石、邊界、敵蛇頭、衝刺蛇或高碰撞風險蛇身。 | 立即放棄目前目標，轉向遠離威脅方向；不可為了吃食物或搶道具主動撞入危險區。 |
  | 2 | 低長度保護 | 當前蛇隻長度 `< AI_PROTECT_MIN_LENGTH`。 | 優先吃近距離安全食物，避免追擊與高風險截斷；衝刺僅可用於脫離危險。 |
  | 3 | 道具爭奪 | 半徑 `AI_DASH_RESOURCE_DISTANCE` 內存在可拾取地圖道具，且路徑未被高風險障礙阻擋。 | 朝道具方向移動；若途中觸發保命迴避，必須中止搶道具。 |
  | 4 | 戰術追擊 / 截斷 | 符合追獵或截斷型 AI 的距離、長度與目標條件。 | 嘗試預判目標前進方向並切入，但超過 `AI_CHASE_ABORT_DISTANCE` 或 `AI_CHASE_ABORT_TIME` 後放棄。 |
...
* **上傳頭像設定**：
  * 玩家可在造型流程中上傳照片作為蛇頭圓形頭像，無等級門檻限制。
  * 上傳後會自動裁切為圓形，並套用至蛇頭中心；蛇身仍使用目前選取的 Skin 美術貼圖。
  * **未上傳**：顯示預設頭像佔位圖，`套用頭像` 按鈕不可用。
  * **預覽中**：上傳後顯示圓形裁切預覽，可重新裁切或套用。
  * **已套用**：成功套用後返回更換外觀頁，蛇頭預覽即時更新。
  * **上傳失敗**：導入現有錯誤提示流程，保留在頭像上傳介面。

![更換外觀上傳頭像示意圖](<./Reference Image/Ref_更換外觀-上傳頭像.png>)
...
* 表情貼圖顯示時長固定為 `EMOTE_DURATION = 2500ms`（由 [16.8 個性化外觀與貼圖參數](#162-module) 控制）。
...
* 該局判定中離後，仍扣除該模式的全額能量門票（`ENTRY_ENERGY_SOLO` / `ENTRY_ENERGY_BRAWL` / `ENTRY_ENERGY_TEAM`）。
...
本章為《貪食蛇》遊戲參數與配置的唯一資料真源。各玩法章節中的來源讀取連結應指向下列對應 DataTable：

- [Ch16.2 Module](#162-module)
- [Ch16.3 AIStrategy](#163-aistrategy)
- [Ch16.4 ActiveSkill](#164-activeskill)
- [Ch16.5 PassiveSkill](#165-passiveskill)
- [Ch16.6 RatingReward](#166-ratingreward)
- [Ch16.7 Food](#167-food)
- [Ch16.8 ModeBotComposition](#168-modebotcomposition)
- [Ch16.9 TutorialLevel](#169-tutoriallevel)
- [Ch16.10 TutorialStep](#1610-tutorialstep)
- [Ch16.11 StringTable](#1611-stringtable)
- [Ch16.12 Skin](#1612-skin)
- [Ch16.13 Emote](#1613-emote)

#### 16.2 Module

> 對應規格：[Ch4 基礎核心](#4-基礎核心與速度控制)｜[Ch6 地圖與環境](#6-地圖與環境系統)｜[Ch8 高級道具](#8-高級道具與覆蓋規則)｜[Ch14 好友房](#14-好友與社交組隊系統)｜[Ch15 異常處理](#15-異常處理與中離邊界條件-自己)
...
> 對應規格：[Ch4.3 蛇隻主動技能規則](#43-蛇隻主動技能規則)｜[Ch11.2 成長之路與技能／道具次數解鎖系統](#112-成長之路與技能道具次數解鎖系統)
...
> 對應規格：[Ch8 高級道具與覆蓋規則](#8-高級道具與覆蓋規則)｜[Ch10 被動養成](#10-被動養成-強化系統)
...
> 對應規格：[Ch3.4.4 單局結算獎勵與評級對照表](#344-單局結算獎勵與評級對照表)
...
> 對應規格：[Ch17 字串表與多語系文字參數](#17-字串表與多語系文字參數)
...
本章收斂遊戲內需要由程式、UI 或營運配置的文字字串。實作時以 [<StringTable表>](#1611-stringtable) 為唯一真源，Key 使用英文固定命名，文字內容提供繁中與英文兩版。
