...
* **兩關制教學**：教學模式僅保留教學關卡 1、2；原本分散於多關的基礎操作整合至關卡 1，高級道具與進階玩法整合至關卡 2。
* **教學關卡技能覆寫**：教學關卡內「暴食」預設直接可用，固定採 Lv 0 數值（吸附半徑 200px、CD 30 秒），不檢查玩家成長之路等級，也不顯示解鎖提示；離開教學後恢復帳號原本的技能解鎖狀態。
* **新手首次通關獎勵**：玩家首次通過教學關卡 1、2 後，系統透過任務系統發放一次性獎勵：500 G 金幣、100 XP 熟練度，並解鎖「經典綠」基礎外觀。
...
graph TD
    ...
    F -->|選擇關卡並進入| G{關卡選擇}
    G -->|關卡 1| H[教學關卡 1 - 基礎操作整合]
    G -->|關卡 2| I[教學關卡 2 - 技能與進階玩法整合]
    H --> N{通關?}
    I --> N
    N -->|是| O[返回教學關卡選擇介面]
    N -->|否| G
    O -->|首次通過關卡 1 與 2| P[領取首次通關獎勵]
    P --> E
```
...
| :---------------------------------------- | :------------------------------------------------------------------ | :-------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------- | :--------------------------------------------------------- |
| **教學關卡 1：基礎操作整合** | 小型封閉沙盒（約 1200×1200 px）；前段無敵人、無地形，後段開放中央橫向河流（寬 80 px）。 | 一關內完成移動、吃食物、理解積分、衝刺、體力消耗與河流減速教學。 | 1. 顯示關卡 1 圖卡。`<br>`2. 左下搖桿區出現手指光暈，引導移動。`<br>`3. 高亮食物並顯示前往虛線；吃到第 3 顆時提示「移動吃食物，可增長蛇身並獲得積分！」。`<br>`4. 吃到 10 顆後開放河流區，右下衝刺按鈕高亮並引導按住。`<br>`5. 衝刺時顯示體力槽與加速特效；進入河流時顯示減速漣漪、蝸牛狀態與提示「河流會降低 50% 速度，按住衝刺可快速通過！」。 | 吃到至少 10 顆食物、個人長度積分達 100 分，並成功衝刺穿越河流 1 次。 | 無。撞牆僅彈回，不致死亦不扣分；體力耗盡時停留於教學步驟並等待回復。 |
| **教學關卡 2：技能與進階玩法整合** | 沙盒地圖；散落高密度食物，配置 2 隻低攻擊性的初階 BOT；不生成隨機實體道具。 | 教導暴食直接使用、暴食 CD，以及磁鐵、鷹眼、巨大蘑菇、幸運糖四種高級道具效果。 | 1. 顯示關卡 2 圖卡。`<br>`2. 暴食按鈕預設可用並高亮，玩家點擊後以 Lv 0 效果吸附食物，按鈕顯示 30 秒 CD；此步驟不檢查成長之路解鎖狀態。`<br>`3. 依序高亮磁鐵、鷹眼、巨大蘑菇、幸運糖按鈕；每項道具提供 1 次免費模擬使用，不扣除帳號金幣與正式對局次數。`<br>`4. 磁鐵吸附食物；鷹眼將視野拉遠至 1.35x 並標記食物；巨大蘑菇放大蛇隻並提供金色無敵光環；幸運糖使食物積分倍率變為 3 倍。`<br>`5. 每完成一項操作即顯示勾選回饋，再自動前進至下一項。 | 至少使用暴食 1 次，並各使用磁鐵、鷹眼、巨大蘑菇、幸運糖 1 次；關卡內個人長度積分達 300 分。 | 無。教學用技能與道具不受帳號解鎖、金幣或單局次數限制；未完成目前步驟時不可跳至下一項。 |
...
* **版型結構**：全畫面覆蓋層依裝置安全區自適應；背景使用關卡實機畫面並加深色遮罩，上方顯示關卡標題，中段排列 3 組圖文重點，底部固定顯示繼續操作。
...
| 圖卡 | 圖片欄位（先製作） | 美術與文字說明 | 狀態／驗收 |
| :--- | :--- | :--- | :--- |
| **關卡 1：基礎操作** | ① 搖桿＋手指拖曳示意。`<br>`② 蛇頭吞食發光食物、蛇身增長示意。`<br>`③ 衝刺按鈕＋體力槽＋穿越河流示意。 | ①「拖曳搖桿控制移動」`<br>`②「吃食物增長並獲得積分」`<br>`③「按住衝刺，快速穿越河流」；以箭頭由上至下串接三段。 | 搖桿、食物、衝刺按鈕及河流可一眼辨識；不可誤讀為點擊移動或點按一次即可持續衝刺；底部顯示「點擊繼續」。 |
| **關卡 2：技能與道具** | ① 暴食按鈕＋範圍吸附食物示意。`<br>`② 磁鐵／鷹眼 Icon 與效果示意。`<br>`③ 巨大蘑菇／幸運糖 Icon 與效果示意。 | ①「點擊暴食，吸附周圍食物」`<br>`②「磁鐵吸食、鷹眼擴大視野」`<br>`③「蘑菇強化、幸運糖提高積分」；暴食置於第一段且視覺權重最高。 | 暴食按鈕呈可用狀態，不得出現鎖頭或解鎖字樣；四種道具 Icon 與效果須能互相對應；不可誤讀為教學使用會扣除正式帳號金幣。 |

##### 3.5.5 退出與跳過機制

* **離開教學**：關卡對局中允許玩家隨時點擊「離開」按鈕，彈出確認提示：「離開教學將返回關卡選擇，尚未完成的進度不會保留，確定離開嗎？」。確認後返回教學關卡選擇介面。
* **重新體驗**：通關後，玩家仍可在關卡選擇介面中自由選擇關卡 1 或 2 重複體驗，但不再重複發放首次通關獎勵。
...
| 教學關卡 1          | 圖卡重點 1                  | `ui_tutorial_l1_move`      | 拖曳搖桿控制移動      | Drag the joystick to move                    |
| 教學關卡 1          | 圖卡重點 2                  | `ui_tutorial_l1_food`      | 吃食物增長並獲得積分  | Eat food to grow and score                   |
| 教學關卡 1          | 圖卡重點 3                  | `ui_tutorial_l1_dash`      | 按住衝刺，快速穿越河流 | Hold Dash to cross the river quickly         |
| 教學關卡 2          | 圖卡重點 1                  | `ui_tutorial_l2_gluttony`  | 點擊暴食，吸附周圍食物 | Tap Gluttony to pull in nearby food           |
| 教學關卡 2          | 圖卡重點 2                  | `ui_tutorial_l2_tools_a`   | 磁鐵吸食、鷹眼擴大視野 | Magnet attracts food; Eagle Eye expands view |
| 教學關卡 2          | 圖卡重點 3                  | `ui_tutorial_l2_tools_b`   | 蘑菇強化、幸運糖提高積分 | Mushroom empowers; Lucky Candy boosts score  |
