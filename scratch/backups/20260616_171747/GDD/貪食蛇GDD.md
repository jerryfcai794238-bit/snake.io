...
* **體力槽 HUD（衝刺體力）**：
  * 顯示目前可用衝刺體力，需與衝刺按鈕視覺關聯，玩家能快速判斷可衝刺時間。
  * **正常可使用 / 消耗中**：體力槽使用黃色，按住衝刺時黃色填充長度隨體力消耗縮短。
  * **超載狀態**：體力歸 0 後體力槽改為紅色，表示暫時不可衝刺；回復完成後恢復黃色可用狀態。
* **暴食按鈕（右下角-小顆 圓形）**：
  * 點擊觸發， N 秒冷卻遮罩與倒計時。
* **道具快捷鍵（中央下排-小顆 方形r角）**：
...
* **介面互動與彈窗規則**：
  ...
  * **全滿級狀態**：當 10 種被動屬性皆達 Lv 40，`強化` 按鈕顯示灰階並改文案為 `已全部滿級`；點擊時不觸發任何抽選，中央顯示漂浮提示 `所有被動屬性已滿級`。
...
| 被動強化 | 按鈕滿級狀態 | `ui_passive_btn_maxed` | 已全部滿級 | All Maxed |
